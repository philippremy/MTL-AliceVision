...
bool ImageDescriber_AKAZE::describe(const image::Image<float>& image, std::unique_ptr<Regions>& regions, const image::Image<unsigned char>* mask)
{
    _params.options.descFactor =
      (_params.akazeDescriptorType == AKAZE_MSURF || _params.akazeDescriptorType == AKAZE_LIOP) ? 10.f * sqrtf(2.f) : 11.f * sqrtf(2.f);  // MLDB
...
#pragma omp parallel for
            for (int i = 0; i < static_cast<int>(keypoints.size()); ++i)
            {
                ...
                ComputeMSURFDescriptor(cur_slice.Lx, cur_slice.Ly, point.octave, regionsCasted->Features()[i], regionsCasted->Descriptors()[i]);
            }
        }
        break;
        case AKAZE_LIOP:
