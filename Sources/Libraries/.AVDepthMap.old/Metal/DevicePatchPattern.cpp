...
DevicePatchPatternArray::DevicePatchPatternArray(uint64_t deviceID)
{
    const auto resMng = MTLDeviceManager::getInstance()->getResourceManager(deviceID);
    this->m_devicePatchPatternBuffer = std::make_unique<MTLSharedResource<MTLBuffer, DevicePatchPattern>>(resMng->createBuffer<DevicePatchPattern>(1, false));
}

const MTLSharedResource<MTLBuffer, DevicePatchPattern>& DevicePatchPatternArray::getDevicePatchPatternBuffer() const
{
    return *this->m_devicePatchPatternBuffer;
}

void DevicePatchPatternArray::fillDevicePatchPatternBuffer(const CustomPatchPatternParams& patchParams)
{
    ...
    this->m_devicePatchPatternBuffer->copyFromHost(&patchPattern_h, 1, 0, 0);
