...
#include <AVMVSData/geometry.hpp>
...
void copyFloat2Map(image::Image<float>& out_mapX,
                   image::Image<float>& out_mapY,
                   const VulkanMemoryBase<float2>& in_map,
                   const ROI& roi,
                   int downscale)
{
    const ROI downscaledROI = downscaleROI(roi, downscale);
    ...
    // Get data for host
    const auto hostData = in_map.getData();
    const auto bufferExtent = in_map.getDimensionSizes().value();
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            const float2& value = hostData[y * bufferExtent.width + x];
            out_mapX(y, x) = value.x;
            out_mapY(y, x) = value.y;
        }
    }
                    ...
                    const std::string& name)
{
    ...
    copyFloat2Map(mapX, mapY, in_map, roi, scaleStep);
...
void writeFloat3Map(int rc,
                    ...
                    const mvsUtils::TileParams& tileParams,
                    const ROI& roi,
                    const VulkanMemoryBase<float3>& in_map,
                    const mvsUtils::EFileType fileType,
                    int scale,
                    int step,
                    const std::string& name)
{
    ...
    for (size_t y = 0; y < size_t(height); ++y)
    {
        for (size_t x = 0; x < size_t(width); ++x)
        {
            const float3& rgba_hmh = hostData[y * bufferExtent.width + x];
            image::RGBfColor& rgb = map(int(y), int(x));
            rgb.r() = rgba_hmh.x;
            rgb.g() = rgba_hmh.y;
            rgb.b() = rgba_hmh.z;
...
void writeFloat4Map(int rc,
                    const mvsUtils::MultiViewParams& mp,
                    const mvsUtils::TileParams& tileParams,
                    const ROI& roi,
                    const VulkanMemoryBase<float4>& in_map,
                    const mvsUtils::EFileType fileType,
                    int scale,
                    int step,
                    const std::string& name)
{
    ...
    for (size_t y = 0; y < size_t(height); ++y)
    {
        for (size_t x = 0; x < size_t(width); ++x)
        {
            const float4& rgba_hmh = hostData[y * bufferExtent.width + x];
            image::RGBfColor& rgb = map(int(y), int(x));
            rgb.r() = rgba_hmh.x;
            rgb.g() = rgba_hmh.y;
            rgb.b() = rgba_hmh.z;
...
void writeDeviceImage(const VulkanMemoryBase<VulkanRGBA>& in_map, const std::string& path)
{
    // copy map from device pitched memory to host memory
    const auto hostData = in_map.getData();
    const auto bufferExtent = in_map.getDimensionSizes().value();
    ...
    // copy image from host memory to an Image
    image::Image<image::RGBfColor> img(bufferExtent.width, bufferExtent.height, true, {0.f, 0.f, 0.f});

    for (size_t y = 0; y < bufferExtent.height; ++y)
    {
        for (size_t x = 0; y < bufferExtent.width; ++x)
        {
            const VulkanRGBA& rgba_hmh = hostData[y * bufferExtent.width + x];
            image::RGBfColor& rgb = img(int(y), int(x));
            rgb.r() = rgba_hmh.x;
            rgb.g() = rgba_hmh.y;
            rgb.b() = rgba_hmh.z;
...
void writeNormalMap(int rc,
                    ...
                    const mvsUtils::TileParams& tileParams,
                    const ROI& roi,
                    const VulkanMemoryBase<float4>& in_normalMap_dmp,
                    int scale,
                    int step,
                    const std::string& name)
{
    writeFloat4Map(rc, mp, tileParams, roi, in_normalMap_dmp, mvsUtils::EFileType::normalMap, scale, step, name);
}
...
void writeNormalMapFiltered(int rc,
                            ...
                            const std::string& name)
{
    writeFloat4Map(rc, mp, tileParams, roi, in_normalMap_dmp, mvsUtils::EFileType::normalMapFiltered, scale, step, name);
}
...
void writeDepthThicknessMap(int rc,
                            ...
                            const mvsUtils::TileParams& tileParams,
                            const ROI& roi,
                            const VulkanMemoryBase<float2>& in_depthThicknessMap_dmp,
                            int scale,
                            int step,
                            const std::string& name)
{
...
void writeDepthPixSizeMap(int rc,
                          ...
                          const mvsUtils::TileParams& tileParams,
                          const ROI& roi,
                          const VulkanMemoryBase<float2>& in_depthPixSize_dmp,
                          int scale,
                          int step,
                          const std::string& name)
{
...
void writeDepthSimMap(int rc,
                      ...
                      const mvsUtils::TileParams& tileParams,
                      const ROI& roi,
                      const VulkanMemoryBase<float2>& in_depthSimMap_dmp,
                      int scale,
                      int step,
                      const std::string& name)
{
...
void writeDepthSimMapFromTileList(int rc,
                                  ...
                                  const mvsUtils::TileParams& tileParams,
                                  const std::vector<ROI>& tileRoiList,
                                  const std::vector<VulkanImage<float2>>& in_depthSimMapTiles_hmh,
                                  int scale,
                                  int step,
                                  const std::string& name)
{
...
void resetDepthSimMap(const VulkanMemoryBase<float2>& inout_depthSimMap_hmh, float depth, float sim)
{

    const auto hostData = inout_depthSimMap_hmh.getData();
    const auto bufferExtent = inout_depthSimMap_hmh.getDimensionSizes().value();

    for (size_t y = 0; y < bufferExtent.height; ++y)
    {
        for (size_t x = 0; y < bufferExtent.width; ++x)
        {
            float2& depthSim_hmh = hostData[y * bufferExtent.width + x];
            depthSim_hmh.x = depth;
            depthSim_hmh.y = sim;
        }
    }

    // Cast to check types and copy back with derived class
    if(inout_depthSimMap_hmh.isVkImage()) {
        const auto& derived = dynamic_cast<const VulkanImage<float2>&>(inout_depthSimMap_hmh);
        derived.copyFromHostToImage(hostData.get(), derived.getSize(), 0, 0);
    } else {
        const auto& derived = dynamic_cast<const VulkanBuffer<float2>&>(inout_depthSimMap_hmh);
        derived.copyFromHostToBuffer(hostData.get(), derived.getSize(), 0, 0);
    }
}
...
void copyFloat2Map(image::Image<float>& out_mapX,
                   image::Image<float>& out_mapY,
                   const MTLSharedResource<MTLTexture, float4>& in_map_hmh,
                   const ROI& roi,
                   int downscale)
{
    const ROI downscaledROI = downscaleROI(roi, downscale);
    ...
    // copy map from device pitched memory to host memory
    const auto hostData = in_map_hmh.getData();
    const auto imageExtent = in_map_hmh.getTextureDimensions();
    ...
    for (int x = 0; x < width; ++x)
    {
        for (int y = 0; y < height; ++y)
        {
            const float4& value = (*hostData)[y * imageExtent.width + x];
            out_mapX(y, x) = value.x;
            out_mapY(y, x) = value.y;
        }
    }
...
void writeFloat3Map(int rc,
                    ...
                    const mvsUtils::TileParams& tileParams,
                    const ROI& roi,
                    const MTLSharedResource<MTLTexture, float4>& in_map_dmp,
                    const mvsUtils::EFileType fileType,
                    int scale,
                    int step,
                    const std::string& name)
{
    ...
    // copy map from device pitched memory to host memory
    const auto hostData = in_map_dmp.getData();
    const auto imageExtent = in_map_dmp.getTextureDimensions();
    ...
    for (size_t x = 0; x < size_t(width); ++x)
    {
        for (size_t y = 0; y < size_t(height); ++y)
        {
            const float4& rgba_hmh = (*hostData)[y * imageExtent.width + x];
            image::RGBfColor& rgb = map(int(y), int(x));
            rgb.r() = rgba_hmh.x;
            rgb.g() = rgba_hmh.y;
            rgb.b() = rgba_hmh.z;
...
void writeDeviceImage(const MTLSharedResource<MTLTexture, MTLRGBA>& in_img_dmp, const std::string& path)
{

    const auto hostData = in_img_dmp.getData();
    const auto imageExtent = in_img_dmp.getTextureDimensions();

    // copy image from host memory to an Image
    image::Image<image::RGBfColor> img(imageExtent.width, imageExtent.height, true, {0.f, 0.f, 0.f});

    for (size_t x = 0; x < imageExtent.width; ++x)
    {
        for (size_t y = 0; y < imageExtent.height; ++y)
        {
            const MTLRGBA& rgba_hmh = (*hostData)[y * imageExtent.width + x];
            image::RGBfColor& rgb = img(int(y), int(x));
            rgb.r() = rgba_hmh.x;
            rgb.g() = rgba_hmh.y;
            rgb.b() = rgba_hmh.z;
...
void writeNormalMap(int rc,
                    ...
                    const mvsUtils::TileParams& tileParams,
                    const ROI& roi,
                    const MTLSharedResource<MTLTexture, float4>& in_normalMap_dmp,
                    int scale,
                    int step,
                    const std::string& name)
{
...
void writeDepthThicknessMap(int rc,
                            ...
                            const mvsUtils::TileParams& tileParams,
                            const ROI& roi,
                            const MTLSharedResource<MTLTexture, float4>& in_depthThicknessMap_dmp,
                            int scale,
                            int step,
                            const std::string& name)
{
...
void writeDepthPixSizeMap(int rc,
                          ...
                          const mvsUtils::TileParams& tileParams,
                          const ROI& roi,
                          const MTLSharedResource<MTLTexture, float4>& in_depthPixSize_dmp,
                          int scale,
                          int step,
                          const std::string& name)
{
...
void writeDepthSimMap(int rc,
                      ...
                      const mvsUtils::TileParams& tileParams,
                      const ROI& roi,
                      const MTLSharedResource<MTLTexture, float4>& in_depthSimMap_dmp,
                      int scale,
                      int step,
                      const std::string& name)
{
...
void writeDepthSimMapFromTileList(int rc,
                                  ...
                                  const mvsUtils::TileParams& tileParams,
                                  const std::vector<ROI>& tileRoiList,
                                  const std::vector<MTLSharedResource<MTLTexture, float4>>& in_depthSimMapTiles_hmh,
                                  int scale,
                                  int step,
                                  const std::string& name)
{
...
void resetDepthSimMap(MTLSharedResource<MTLTexture, float4>& inout_depthSimMap_hmh, float depth, float sim)
{
    const auto hostData = inout_depthSimMap_hmh.getData();
    const auto depthSimMapSize = inout_depthSimMap_hmh.getTextureDimensions();

    for (size_t x = 0; x < depthSimMapSize.width; ++x)
    {
        for (size_t y = 0; y < depthSimMapSize.height; ++y)
        {
            float4& depthSim_hmh = (*hostData)[y * depthSimMapSize.width + x];
            depthSim_hmh.x = depth;
            depthSim_hmh.y = sim;
        }
    }
    ...
    // Write back
    inout_depthSimMap_hmh.copyFromHost(hostData->data(), hostData->size(), 0, 0);
}

#else
    #error No backend for DepthMap computations enabled!
#endif
