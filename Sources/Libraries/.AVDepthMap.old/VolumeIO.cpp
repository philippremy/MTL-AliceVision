...
#include <AVMVSData/Matrix3x3.hpp>
#include <AVMVSData/Matrix3x4.hpp>
#include <AVMVSData/OrientedPoint.hpp>
#include <AVMVSData/geometry.hpp>
#include <AVImage/jetColorMap.hpp>
#include <AVMVSUtils/common.hpp>
                                ...
                                const ROI& roi)
{
    ...
    // Get host visible buffer
    const auto hostBuffer = in_volumeSim_hmh.getData();
    const auto imageExtent = in_volumeSim_hmh.getDimensionSizes().value();
    ...
    for (int iy = 0; iy < sampleSize; ++iy)
    {
        for (int ix = 0; ix < sampleSize; ++ix)
        {
            ...
            for (int iz = 0; iz < in_depths.size(); ++iz)
            {
                const float simValue = float(hostBuffer[iz * (imageExtent.height * imageExtent.width) + iy * imageExtent.width + ix]);
                pDepths.push_back(simValue);
            }
        }
    }
                                ...
                                const ROI& roi)
{
    ...
    for (int iy = 0; iy < sampleSize; ++iy)
    {
        for (int ix = 0; ix < sampleSize; ++ix)
        {
            ...
            pDepths.reserve(imageExtent.depth);

            for (int iz = 0; iz < imageExtent.depth; ++iz)
            {
                const float simValue = float(hostBuffer[iz * (imageExtent.height * imageExtent.width) + iy * imageExtent.width + ix]);
                pDepths.push_back(simValue);
            }
                            ...
                            const ROI& roi)
{
    ...
    for (int vy = 0; vy < imageExtent.height; vy += xyStep)
    {
        for (int vx = 0; vx <imageExtent.width; vx += xyStep)
        {
            const double x = roi.x.begin + (vx * sgmParams.scale * sgmParams.stepXY);
            const double y = roi.y.begin + (vy * sgmParams.scale * sgmParams.stepXY);
            ...
            for (int vz = 0; vz < in_depths.size(); ++vz)
            {
                ...
                const float maxValue = 80.f;
                float simValue = hostBuffer[vz * (imageExtent.height * imageExtent.width) + vy * imageExtent.width + vx];
                if (simValue > maxValue)
                    continue;
                const rgb c = getRGBFromJetColorMap(simValue / maxValue);
                pointCloud.getLandmarks()[landmarkId] =
                                 ...
                                 const ROI& roi)
{
    ...
    for (int vz = 0; vz < in_depths.size(); ++vz)
    {
        for (int vy = 0; vy < imageExtent.height; ++vy)
        {
            const bool vyCenter = (vy >= imageExtent.height / 2) && ((vy - 1) < imageExtent.height / 2);
            const int xIdxStart = (vyCenter ? 0 : (imageExtent.width / 2));
            const int xIdxStop = (vyCenter ? imageExtent.width : (xIdxStart + 1));
                                 ...
                                 const ROI& roi)
{
    ...
    // Get host visible buffer
    const auto hostBuffer2 = in_depthSimMapSgmUpscale_hmh.getData();
    const auto imageExtent2 = in_depthSimMapSgmUpscale_hmh.getDimensionSizes().value();

    IndexT landmarkId = 0;

    for (int vy = 0; vy < imageExtent.height; ++vy)
    {
        const bool vyCenter = ((vy * 2) == imageExtent.height);
        const int xIdxStart = (vyCenter ? 0 : (imageExtent.width / 2));
        const int xIdxStop = (vyCenter ? imageExtent.width : (xIdxStart + 1));
        ...
        for (int vx = xIdxStart; vx < xIdxStop; ++vx)
        {
            ...
            const float2 depthPixSizeMap = hostBuffer2[vy * imageExtent2.width + vx];

            if (depthPixSizeMap.x < 0.0f)  // original depth invalid or masked
                continue;

            for (int vz = 0; vz < imageExtent.depth; ++vz)
            {
                const float simValue = float(hostBuffer[vz * (imageExtent.height * imageExtent.width) + vy * imageExtent.width + vx]);
                                          ...
                                          const ROI& roi)
{
    ...
    const size_t vy = size_t(divideRoundUp(int(imageExtent.height), 2));  // center only
    ...
    for (size_t vx = 0; vx < imageExtent.width; ++vx)
    {
        for (size_t vz = 0; vz < imageExtent.depth; ++vz)
        {
            const float simValue = float(hostBuffer[vz * (imageExtent.height * imageExtent.width) + vy * imageExtent.width + vx]);
                                          ...
                                          const ROI& roi)
{
    ...
    for (size_t vx = 0; vx < imageExtent.width; ++vx)
    {
        ...
        for (size_t vz = 0; vz < imageExtent.width; ++vz)
        {
            const float simValue = float(hostBuffer[vz * (imageExtent.height * imageExtent.width) + vy * imageExtent.width + vx]);
            const float simValueNorm = (simValue - minSim) / (maxSim - minSim);
            const float simValueColor = 1 - simValueNorm;  // best similarity value is 0, worst value is 1
                       ...
                       const ROI& roi)
{
    ...
    ALICEVISION_LOG_DEBUG("DepthMap exportColorVolume: " << imageExtent.width << " x " << imageExtent.height << " x " << nbDepths << ", volDim[2]=" << imageExtent.depth
                                                         << ", xyStep=" << xyStep << ".");
    ...
    for (int vy = 0; vy < imageExtent.height; vy += xyStep)
    {
        for (int vx = 0; vx < imageExtent.width; vx += xyStep)
        {
            ...
            for (int vz = 0; vz < nbDepths; ++vz)
            {
                ...
                float4 colorValue = hostBuffer[vz * (imageExtent.height * imageExtent.width) + vy * imageExtent.width + vx];
                const rgb c = float4_to_rgb(colorValue);  // TODO: convert Lab color into sRGB color
                pointCloud.getLandmarks()[landmarkId] =
                  sfmData::Landmark(Vec3(p.x, p.y, p.z), feature::EImageDescriberType::UNKNOWN, image::RGBColor(c.r, c.g, c.b));
                                ...
                                const ROI& roi)
{
    ...
    // Get host visible buffer
    const auto hostBuffer = in_volumeSim_hmh.getData();
    const auto imageExtent = in_volumeSim_hmh.getTextureDimensions();
                                 ...
                                 const ROI& roi)
{
    ...
    // Get host visible buffer
    const auto hostBuffer2 = in_depthSimMapSgmUpscale_hmh.getData();
    const auto imageExtent2 = in_depthSimMapSgmUpscale_hmh.getTextureDimensions();
    ...
    for (int vy = 0; vy < imageExtent.height; ++vy)
    {
        ...
        for (int vx = xIdxStart; vx < xIdxStop; ++vx)
        {
            ...
            const float4 depthPixSizeMap = (*hostBuffer2)[vy * imageExtent2.width + vx];
