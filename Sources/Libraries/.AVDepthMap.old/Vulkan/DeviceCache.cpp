 ...
 */
void fillDeviceCameraParameters(uint32_t deviceID, const DeviceCameraParams& cameraParameters_h, int deviceCameraParamsId)
{
    DeviceCameraParamsArray::getInstance(deviceID)->getCameraParamsArray().copyFromHostToBuffer(&cameraParameters_h, 1, 0, deviceCameraParamsId * sizeof(DeviceCameraParams));
}

DeviceCache::SingleDeviceCache::SingleDeviceCache(uint32_t deviceID, int maxMipmapImages, int maxCameraParams)
  : deviceID(deviceID),
    ...
    cameraParamCache(maxCameraParams)
{
    ...
    DeviceGaussianArray::getInstance(deviceID)->createConstantGaussianArray(DEVICE_MAX_DOWNSCALE);
...
void DeviceCache::clear(uint32_t deviceID)
{
...
void DeviceCache::build(uint32_t deviceID, int maxMipmapImages, int maxCameraParams)
{

    // reset the current device cache
    _cachePerDevice[deviceID].reset(new SingleDeviceCache(deviceID, maxMipmapImages, maxCameraParams));
}

DeviceCache::SingleDeviceCache& DeviceCache::getCurrentDeviceCache(uint32_t deviceID)
{
    // find the current SingleDeviceCache
    auto it = _cachePerDevice.find(deviceID);
...
void DeviceCache::addMipmapImage(uint32_t deviceID,
                                 int camId,
                                 int minDownscale,
                                 int maxDownscale,
...
#endif
    VulkanImage<VulkanRGBA> img_buf = VulkanMemoryBase<VulkanRGBA>::create2DImage(deviceID, img->width(), img->height(), imageFormat, log2(maxDownscale / minDownscale) + 1, true, false);

    // Create host buffer
    std::vector<VulkanRGBA> host_buffer = {};
    host_buffer.reserve(img->width() * img->height());
...
#pragma omp parallel for
    for (int y = 0; y < img->height(); ++y)
    {
        for (int x = 0; x < img->width(); ++x)
        {
            ...
            // explicit float to unsigned int conversion
            cudaRGBA.x = static_cast<unsigned int>(floatRGBA.r() * 255.0f);
            cudaRGBA.y = static_cast<unsigned int>(floatRGBA.g() * 255.0f);
            cudaRGBA.z = static_cast<unsigned int>(floatRGBA.b() * 255.0f);
            cudaRGBA.w = static_cast<unsigned int>(floatRGBA.a() * 255.0f);
#elif defined(ALICEVISION_DEPTHMAP_TEXTURE_USE_HALF)
            // explicit float to half conversion
            cudaRGBA.x = float2half(floatRGBA.r() * 255.0f);
            cudaRGBA.y = float2half(floatRGBA.g() * 255.0f);
            cudaRGBA.z = float2half(floatRGBA.b() * 255.0f);
            cudaRGBA.w = float2half(floatRGBA.a() * 255.0f);
#else
...
#endif
            host_buffer.push_back(cudaRGBA);
        }
    }

    // Copy host data into device buffer
    img_buf.copyFromHostToImage(host_buffer.data(), host_buffer.size(), 0, 0);

    // Setup mipmap cache
    DeviceMipmapImage& deviceMipmapImage = *(currentDeviceCache.mipmaps.at(deviceMipmapId));
    deviceMipmapImage.fill(img_buf, minDownscale, maxDownscale);
}

void DeviceCache::addCameraParams(uint32_t deviceID, int camId, int downscale, const mvsUtils::MultiViewParams& mp)
{
    // get current device cache
    SingleDeviceCache& currentDeviceCache = getCurrentDeviceCache(deviceID);
...
const DeviceMipmapImage& DeviceCache::requestMipmapImage(uint32_t deviceID, int camId, const mvsUtils::MultiViewParams& mp)
{
    // get current device cache
    SingleDeviceCache& currentDeviceCache = getCurrentDeviceCache(deviceID);
...
const int DeviceCache::requestCameraParamsId(uint32_t deviceID, int camId, int downscale, const mvsUtils::MultiViewParams& mp)
{
    // get current device cache
    SingleDeviceCache& currentDeviceCache = getCurrentDeviceCache(deviceID);
