...
#include <AVDepthMap/DepthMapEstimator.hpp>
...
int DepthMapEstimator::getNbSimultaneousTiles(uint32_t deviceID) const
{
    const int nbTilesPerCamera = _tileRoiList.size();
    ...
    // TODO: special case first mipmap level != 1
    const double mipmapCostMB =
      ((_mp.getMaxImageWidth() * 1.5) * _mp.getMaxImageHeight() * sizeof(VulkanRGBA)) / (1024.0 * 1024.0);  // process downscale apply
    ...
    // number of camera parameters in device constant memory
    // (Rc + Tcs) * 2 (SGM + Refine downscale) + 1 (SGM needs downscale 1)
    // note: special case SGM downscale = Refine downscale not handle
    const int rcNbCameraParams = (_depthMapParams.useRefine) ? 2 : 1;
    const int rcCamParams = (1 /* rc */ + _depthMapParams.maxTCams) * rcNbCameraParams + ((_refineParams.scale > 1) ? 1 : 0);
...

    {
        ...
        Sgm sgm(_mp, _tileParams, _sgmParams, sgmComputeDepthSimMap, sgmComputeNormalMap, deviceID /*device*/);
        sgmTileCostMB = sgm.getDeviceMemoryConsumption();
    ...
    if (_depthMapParams.useRefine)
    {
        Refine refine(_mp, _tileParams, _refineParams, deviceID /*device*/);
        refineTileCostMB = refine.getDeviceMemoryConsumption();
    ...
    // available device memory
    double deviceMemoryMB = getAvailableMemory(deviceID) * 0.8;
...
void DepthMapEstimator::compute(uint32_t deviceID, const std::vector<int>& cams)
{
    // initialize RAM image cache
    // note: maybe move it as class member in order to share it across multiple GPUs
    mvsUtils::ImagesCache<image::Image<image::RGBAfColor>> ic(_mp, image::EImageColorSpace::LINEAR);
    ...
    // get maximum number of simultaneous tiles
    // for now, we use one CUDA stream per tile (SGM + Refine)
    const int nbStreams = std::min(getNbSimultaneousTiles(deviceID), static_cast<int>(tiles.size()));
    ...
    DeviceCache& deviceCache = DeviceCache::getInstance();
    deviceCache.build(deviceID, nbMipmapImagesPerBatch, nbCamerasParamsPerBatch);

    // build custom patch pattern in CUDA constant memory
    if (_sgmParams.useCustomPatchPattern || _refineParams.useCustomPatchPattern)
        buildCustomPatchPattern(deviceID, _depthMapParams.customPatchPattern);
    ...
    // initialize Sgm and Refine objects
    {
        ...
        // initialize Sgm objects
        for (int i = 0; i < nbStreams; ++i)
            sgmPerStream.emplace_back(_mp, _tileParams, _sgmParams, sgmComputeDepthSimMap, sgmComputeNormalMap, deviceID);

        // initialize Refine objects
        if (_depthMapParams.useRefine)
            for (int i = 0; i < nbStreams; ++i)
                refinePerStream.emplace_back(_mp, _tileParams, _refineParams, deviceID);
    }

    // allocate final deth/similarity map tile list in host memory
    std::vector<std::vector<std::shared_ptr<VulkanImage<float2>>>> depthSimMapTilePerCam(nbRcPerBatch);
    std::vector<std::vector<std::pair<float, float>>> depthMinMaxTilePerCam(nbRcPerBatch);
    ...
    for (int i = 0; i < nbRcPerBatch; ++i)
    {
        ...
        {
            if (_depthMapParams.useRefine)
                depthSimMapTiles.at(j) = std::make_shared<VulkanImage<float2>>(VulkanMemoryBase<float2>::create2DImage(deviceID, refinePerStream.front().getDeviceDepthSimMap().getDimensionSizes().value().width, refinePerStream.front().getDeviceDepthSimMap().getDimensionSizes().value().height, vk::Format::eR32G32Sfloat, 1, false, true));
            else  // final depth/similarity map is SGM only
                depthSimMapTiles.at(j) = std::make_shared<VulkanImage<float2>>(VulkanMemoryBase<float2>::create2DImage(deviceID, sgmPerStream.front().getDeviceDepthSimMap().getDimensionSizes().value().width, sgmPerStream.front().getDeviceDepthSimMap().getDimensionSizes().value().height, vk::Format::eR32G32Sfloat, 1, false, true));
        }
    }
    ...
    for (int b = 0; b < nbBatches; ++b)
    {
        ...
        for (int i = firstTileIndex; i < lastTileIndex; ++i)
        {
            ...
            // add Sgm R camera to Device cache
            deviceCache.addMipmapImage(deviceID, tile.rc, minMipmapDownscale, maxMipmapDownscale, ic, _mp);
            deviceCache.addCameraParams(deviceID, tile.rc, _sgmParams.scale, _mp);

            // add Sgm T cameras to Device cache
            for (const int tc : tile.sgmTCams)
            {
                deviceCache.addMipmapImage(deviceID, tc, minMipmapDownscale, maxMipmapDownscale, ic, _mp);
                deviceCache.addCameraParams(deviceID, tc, _sgmParams.scale, _mp);
            }

            if (_depthMapParams.useRefine)
            {
                // add Refine R camera to Device cache
                deviceCache.addCameraParams(deviceID, tile.rc, _refineParams.scale, _mp);

                // add Refine T cameras to Device cache
                for (const int tc : tile.refineTCams)
                {
                    deviceCache.addMipmapImage(deviceID, tc, minMipmapDownscale, maxMipmapDownscale, ic, _mp);
                    deviceCache.addCameraParams(deviceID, tc, _refineParams.scale, _mp);
                }
            }
            ...
            if (!hasRcWithoutDownscale)
            {
                // add SGM R camera at scale 1 to Device cache.
                // R camera parameters at scale 1 are required for SGM retrieve best depth
                deviceCache.addCameraParams(deviceID, tile.rc, 1, _mp);
            }
        }
        ...
        for (int i = firstTileIndex; i < lastTileIndex; ++i)
        {
            ...
            // get tile result depth/similarity map in host memory
            auto& tileDepthSimMap_hmh = depthSimMapTilePerCam.at(batchCamIndex).at(tile.id);
            ...
            if (_depthMapParams.useRefine)
            {
                ...
                // compute Refine
                Refine& refine = refinePerStream.at(streamIndex);
                refine.refineRc(tile, dynamic_cast<const VulkanImage<float2>&>(sgm.getDeviceDepthThicknessMap()), dynamic_cast<const VulkanImage<float4>&>(sgm.getDeviceNormalMap()));

                // copy Refine depth/similarity map from device to host
                tileDepthSimMap_hmh->copyFromAllocation(refine.getDeviceDepthSimMap(), 0, 0);
            }
            else
            {
                // copy Sgm depth/similarity map from device to host
                tileDepthSimMap_hmh->copyFromAllocation(sgm.getDeviceDepthSimMap(), 0, 0);
            }
        }
        ...
        for (int c = firstRc; c <= lastRc; ++c)
        {
            ...
            // Create actual vectors
            std::vector<VulkanImage<float2>> depthSimMapTilePerCamIdx;
            depthSimMapTilePerCamIdx.reserve(depthSimMapTilePerCam.at(batchCamIndex).size());
            for(auto temp : depthSimMapTilePerCam.at(batchCamIndex)) {
                depthSimMapTilePerCamIdx.push_back(*temp);
            }

            if (_depthMapParams.useRefine)
                writeDepthSimMapFromTileList(
                  c, _mp, _tileParams, _tileRoiList, depthSimMapTilePerCamIdx, _refineParams.scale, _refineParams.stepXY);
            else
                writeDepthSimMapFromTileList(
                  c, _mp, _tileParams, _tileRoiList, depthSimMapTilePerCamIdx, _sgmParams.scale, _sgmParams.stepXY);
...
int DepthMapEstimator::getNbSimultaneousTiles(const uint64_t deviceID) const
{
    const int nbTilesPerCamera = _tileRoiList.size();
    ...
    // TODO: special case first mipmap level != 1
    const double mipmapCostMB =
      ((_mp.getMaxImageWidth() * 1.5) * _mp.getMaxImageHeight() * sizeof(MTLRGBA)) / (1024.0 * 1024.0);  // process downscale apply
...

    {
        ...
        Sgm sgm(_mp, _tileParams, _sgmParams, sgmComputeDepthSimMap, sgmComputeNormalMap, deviceID);
        sgmTileCostMB = sgm.getDeviceMemoryConsumption();
        sgmTileCostUnpaddedMB = sgm.getDeviceMemoryConsumptionUnpadded();
    }
    ...
    if (_depthMapParams.useRefine)
    {
        Refine refine(_mp, _tileParams, _refineParams, deviceID);
        refineTileCostMB = refine.getDeviceMemoryConsumption();
        refineTileCostUnpaddedMB = refine.getDeviceMemoryConsumptionUnpadded();
    }
    ...
    // Get device manager
    const auto deviceManager = gpu::MTLDeviceManager::getInstance();

    // available device memory
    double deviceMemoryMB;
    {
        double availableMB, usedMB, totalMB;
        deviceManager->getDeviceMemoryInfo(deviceID, availableMB, usedMB, totalMB);
        deviceMemoryMB = availableMB * 0.8;  // available memory margin
    }
...
void DepthMapEstimator::compute(const uint64_t mtlDeviceID, const std::vector<int>& cams)
{
    // initialize RAM image cache
    // note: maybe move it as class member in order to share it across multiple GPUs
    mvsUtils::ImagesCache<image::Image<image::RGBAfColor>> ic(_mp, image::EImageColorSpace::LINEAR);
    ...
    // get maximum number of simultaneous tiles
    // for now, we use one CUDA stream per tile (SGM + Refine)
    const int nbStreams = std::min(getNbSimultaneousTiles(mtlDeviceID), static_cast<int>(tiles.size()));
    ...
    DeviceCache& deviceCache = DeviceCache::getInstance();
    if (_sgmParams.useCustomPatchPattern)
        deviceCache.build(mtlDeviceID, nbMipmapImagesPerBatch, nbCamerasParamsPerBatch, _depthMapParams.customPatchPattern);
    else
        deviceCache.build(mtlDeviceID, nbMipmapImagesPerBatch, nbCamerasParamsPerBatch, std::nullopt);
    ...
    // allocate final deth/similarity map tile list in host memory
    std::vector<std::vector<std::unique_ptr<MTLSharedResource<MTLTexture, float4>>>> depthSimMapTilePerCam(nbRcPerBatch);
    std::vector<std::vector<std::pair<float, float>>> depthMinMaxTilePerCam(nbRcPerBatch);
    ...
    for (int i = 0; i < nbRcPerBatch; ++i)
    {
        ...
        for (int j = 0; j < nbTilesPerCamera; ++j)
        {
            if (_depthMapParams.useRefine)
            {
                const auto size = refinePerStream.front().getDeviceDepthSimMap().getTextureDimensions();
                depthSimMapTiles[j] = std::make_unique<MTLSharedResource<MTLTexture, float4>>(resMng->createTexture2D<float4>(MTL::PixelFormatRGBA32Float, size.width, size.height, 1, true));
            }
            else  // final depth/similarity map is SGM only
            {
                const auto size = sgmPerStream.front().getDeviceDepthSimMap().getTextureDimensions();
                depthSimMapTiles[j] = std::make_unique<MTLSharedResource<MTLTexture, float4>>(resMng->createTexture2D<float4>(MTL::PixelFormatRGBA32Float, size.width, size.height, 1, true));
            }
        }
    }
    ...
    // log device memory information
    MTLDeviceManager::getInstance()->logDeviceMemoryInfo(mtlDeviceID);
    ...
    for (int b = 0; b < nbBatches; ++b)
    {
        ...
        for (int i = firstTileIndex; i < lastTileIndex; ++i)
        {
            ...
            // add Sgm R camera to Device cache
            deviceCache.addMipmapImage(mtlDeviceID, tile.rc, minMipmapDownscale, maxMipmapDownscale, ic, _mp);
            deviceCache.addCameraParams(mtlDeviceID, tile.rc, _sgmParams.scale, _mp);

            // add Sgm T cameras to Device cache
            for (const int tc : tile.sgmTCams)
            {
                deviceCache.addMipmapImage(mtlDeviceID, tc, minMipmapDownscale, maxMipmapDownscale, ic, _mp);
                deviceCache.addCameraParams(mtlDeviceID, tc, _sgmParams.scale, _mp);
            }

            if (_depthMapParams.useRefine)
            {
                // add Refine R camera to Device cache
                deviceCache.addCameraParams(mtlDeviceID, tile.rc, _refineParams.scale, _mp);

                // add Refine T cameras to Device cache
                for (const int tc : tile.refineTCams)
                {
                    deviceCache.addMipmapImage(mtlDeviceID, tc, minMipmapDownscale, maxMipmapDownscale, ic, _mp);
                    deviceCache.addCameraParams(mtlDeviceID, tc, _refineParams.scale, _mp);
                }
            }
            ...
            if (!hasRcWithoutDownscale)
            {
                // add SGM R camera at scale 1 to Device cache.
                // R camera parameters at scale 1 are required for SGM retrieve best depth
                deviceCache.addCameraParams(mtlDeviceID, tile.rc, 1, _mp);
            }
        }
        ...
        for (int i = firstTileIndex; i < lastTileIndex; ++i)
        {
            ...
            // get tile result depth/similarity map in host memory
            MTLSharedResource<MTLTexture, float4>& tileDepthSimMap_hmh = *depthSimMapTilePerCam.at(batchCamIndex).at(tile.id);
            ...
            if (_depthMapParams.useRefine)
            {
                ...
                // copy Refine depth/similarity map from device to host
                tileDepthSimMap_hmh.copyFromResource(refine.getDeviceDepthSimMap(), 0, 0);
            }
            else
            {
                // copy Sgm depth/similarity map from device to host
                tileDepthSimMap_hmh.copyFromResource(sgm.getDeviceDepthSimMap(), 0, 0);
            }
        }
        ...
        // Out of the unique ptrs
        // Should be inexpensive, as this is mostly increasing reference counts
        std::vector<std::vector<MTLSharedResource<MTLTexture, float4>>> inDepthSimMaps;
        inDepthSimMaps.reserve(depthSimMapTilePerCam.size());
        for (const auto& depthSimMapTilePerCamElement : depthSimMapTilePerCam)
        {
            std::vector<MTLSharedResource<MTLTexture, float4>> innerVec;
            innerVec.reserve(depthSimMapTilePerCamElement.size());

            for (const auto& uniqueResource : depthSimMapTilePerCamElement)
            {
                innerVec.push_back(*uniqueResource);
            }

            inDepthSimMaps.push_back(std::move(innerVec));
        }
        ...
        for (int c = firstRc; c <= lastRc; ++c)
        {
            ...
            if (_depthMapParams.useRefine)
                writeDepthSimMapFromTileList(
                  c, _mp, _tileParams, _tileRoiList, inDepthSimMaps.at(batchCamIndex), _refineParams.scale, _refineParams.stepXY);
            else
                writeDepthSimMapFromTileList(
                  c, _mp, _tileParams, _tileRoiList, inDepthSimMaps.at(batchCamIndex), _sgmParams.scale, _sgmParams.stepXY);
