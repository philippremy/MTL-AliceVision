...
inline void MSKAPI printstr(void* handle, const char str[]) { ALICEVISION_LOG_DEBUG(str); }

MSKenv_t env = nullptr;

MOSEKSolver::MOSEKSolver(int nbParams)
  : ISolver(nbParams)
{
    task = nullptr;
    // env  = nullptr;
    ...
    if (env == nullptr)
    {
        MSKrescodee r = MSK_makeenv(&env, nullptr);
