                     ...
                     const std::string& outputSVGPath)
{
    ...
    for (const auto& descMatches : matches)
    {
        ...
        {
            // Get back linked feature, draw a circle and link them by a line
            std::vector<feature::PointFeature> regionsLeft = keypointsLeft.at(descType)->GetRegionsPositions();
            std::vector<feature::PointFeature> regionsRight = keypointsRight.at(descType)->GetRegionsPositions();
            const feature::PointFeature& L = regionsLeft[m._i];
            const feature::PointFeature& R = regionsRight[m._j];
