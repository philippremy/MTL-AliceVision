...
void Sgm::sgmRc(const Tile& tile, const SgmDepthList& tileDepthList)
{
    ...
    _depths_dmp.copyFrom(_depths_hmh);
    ...
    if (_computeNormalMap)
    {
        ...
        const int rcDeviceCameraParamsId = deviceCache.requestCameraParamsId(tile.rc, _sgmParams.scale, _mp, _deviceID);

        ALICEVISION_LOG_INFO(tile << "SGM compute normal map of view id: " << viewId << ", rc: " << tile.rc << " (" << (tile.rc + 1) << " / "
                                  << _mp.ncams << ").");
        mtl_depthSimMapComputeNormal(_normalMap_dmp, _depthSimMap_dmp, rcDeviceCameraParamsId, _sgmParams.stepXY, downscaledRoi, _deviceID);
...
void Sgm::smoothThicknessMap(const Tile& tile, const RefineParams& refineParams)
{
    ...
    mtl_depthThicknessSmoothThickness(_depthThicknessMap_dmp, _sgmParams, refineParams, downscaledRoi, _deviceID);
...
void Sgm::computeSimilarityVolumes(const Tile& tile, const SgmDepthList& tileDepthList)
{
    ...
    MTLHostMemoryHeap<TSim, 3> volumeSim_hmh(_volumeBestSim_dmp.getSize());
    for (size_t idx=0; idx < volumeSim_hmh.getUnitsTotal(); idx++)
    {
        volumeSim_hmh.getBuffer()[idx] = static_cast<TSim>(255.f);
    }
    _volumeBestSim_dmp.copyFrom(volumeSim_hmh);
    volumeSim_hmh.deallocate();

    MTLHostMemoryHeap<TSim, 3> volumeSecBestSim_hmh(_volumeSecBestSim_dmp.getSize());
    for (size_t idx=0; idx < _volumeSecBestSim_dmp.getUnitsTotal(); idx++)
    {
        volumeSecBestSim_hmh.getBuffer()[idx] = static_cast<TSim>(255.f);
    }
    _volumeSecBestSim_dmp.copyFrom(volumeSecBestSim_hmh);
    volumeSecBestSim_hmh.deallocate();
    ...
    for (std::size_t tci = 0; tci < tile.sgmTCams.size(); ++tci)
    {
        ...
        const int tcDeviceCameraParamsId = deviceCache.requestCameraParamsId(tc, _sgmParams.scale, _mp, _deviceID);
        ...
        mtl_volumeComputeSimilarity(_volumeBestSim_dmp,
    ...
    if (_sgmParams.updateUninitializedSim)  // should always be true, false for debug purposes
    {
        ...
        mtl_volumeUpdateUninitializedSimilarity(_volumeBestSim_dmp, _volumeSecBestSim_dmp, _deviceID);
    }
...
void Sgm::optimizeSimilarityVolume(const Tile& tile, const SgmDepthList& tileDepthList)
{
    ...
    mtl_volumeOptimize(_volumeBestSim_dmp,     // output volume (reuse best sim to put optimized similarity)
...
void Sgm::retrieveBestDepth(const Tile& tile, const SgmDepthList& tileDepthList)
{
    ...
    // get R device camera parameters id from cache
    DeviceCache& deviceCache = DeviceCache::getInstance();
    const int rcDeviceCameraParamsId = deviceCache.requestCameraParamsId(tile.rc, 1, _mp, _deviceID);

    mtl_volumeRetrieveBestDepth(_depthThicknessMap_dmp,  // output depth thickness map
                                 _computeDepthSimMap ? &_depthSimMap_dmp : nullptr,        // output depth/sim map (or empty)
                                  ...
                                  const std::string& name) const
{
    ...
    MTLHostMemoryHeap<TSim, 3> volumeSim_hmh(in_volume_dmp.getSize());
