...
void DepthMapEstimator::compute(uint64_t mtlDeviceID, const std::vector<int>& cams)
{
    ...
    deviceCache.build(nbMipmapImagesPerBatch, nbCamerasParamsPerBatch, mtlDeviceID);
    ...
    std::vector<std::vector<MTLHostMemoryHeap<float2, 2>>> depthSimMapTilePerCam(nbRcPerBatch);
    ...
    logDeviceMemoryInfo(mtlDeviceID);
    ...
    for (int b = 0; b < nbBatches; ++b)
    {
        ...
        for (int i = firstTileIndex; i < lastTileIndex; ++i)
        {
            ...
            deviceCache.addMipmapImage(tile.rc, minMipmapDownscale, maxMipmapDownscale, ic, _mp, mtlDeviceID);
            deviceCache.addCameraParams(tile.rc, _sgmParams.scale, _mp, mtlDeviceID);

            // add Sgm T cameras to Device cache
            for (const int tc : tile.sgmTCams)
            {
                deviceCache.addMipmapImage(tc, minMipmapDownscale, maxMipmapDownscale, ic, _mp, mtlDeviceID);
                deviceCache.addCameraParams(tc, _sgmParams.scale, _mp, mtlDeviceID);
            }

            if (_depthMapParams.useRefine)
            {
                // add Refine R camera to Device cache
                deviceCache.addCameraParams(tile.rc, _refineParams.scale, _mp, mtlDeviceID);

                // add Refine T cameras to Device cache
                for (const int tc : tile.refineTCams)
                {
                    deviceCache.addMipmapImage(tc, minMipmapDownscale, maxMipmapDownscale, ic, _mp, mtlDeviceID);
                    deviceCache.addCameraParams(tc, _refineParams.scale, _mp, mtlDeviceID);
                }
            }
            ...
            if (!hasRcWithoutDownscale)
            {
                ...
                deviceCache.addCameraParams(tile.rc, 1, _mp, mtlDeviceID);
            }
        }
