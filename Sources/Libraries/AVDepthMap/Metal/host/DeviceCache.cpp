 ...
 */
inline void fillDeviceCameraParameters(DeviceCameraParamsManager& mng, const DeviceCameraParams& cameraParameters_h, int deviceCameraParamsId)
{
    mng.insertCameraParams(cameraParameters_h, deviceCameraParamsId);
}

DeviceCache::SingleDeviceCache::SingleDeviceCache(int maxMipmapImages, int maxCameraParams, uint64_t deviceID)
    ...
    cameraParamCache(maxCameraParams)
{
    ...
    this->camParamMng = DeviceCameraParamsManager(deviceID);
    this->gaussMng = DeviceGaussianFilterManager();
    ...
    mtl_createConstantGaussianArray(this->gaussMng, deviceID, DEVICE_MAX_DOWNSCALE);
...
void DeviceCache::clear(uint64_t deviceID)
{
...
void DeviceCache::build(int maxMipmapImages, int maxCameraParams, uint64_t deviceID)
{
    // reset the current device cache
    _cachePerDevice[deviceID].reset(new SingleDeviceCache(maxMipmapImages, maxCameraParams, deviceID));
}

DeviceCache::SingleDeviceCache& DeviceCache::getCurrentDeviceCache(uint64_t deviceID)
{
    // find the current SingleDeviceCache
    auto it = _cachePerDevice.find(deviceID);
                                 ...
                                 uint64_t deviceID)
{
    ...
    MTLSize<2> imgSize(img->width(), img->height());
    MTLHostMemoryHeap<MTLRGBA, 2> img_hmh(imgSize);
...
#pragma omp parallel for
    for (int y = 0; y < imgSize.y(); ++y)
    {
        for (int x = 0; x < imgSize.x(); ++x)
        {
            ...
            MTLRGBA& cudaRGBA = img_hmh(x, y);

#ifdef ALICEVISION_DEPTHMAP_TEXTURE_USE_HALF
            // explicit float to half conversion
            cudaRGBA.x = static_cast<_Float16>(floatRGBA.r() * 255.0f);
            cudaRGBA.y = static_cast<_Float16>(floatRGBA.g() * 255.0f);
            cudaRGBA.z = static_cast<_Float16>(floatRGBA.b() * 255.0f);
            cudaRGBA.w = static_cast<_Float16>(floatRGBA.a() * 255.0f);
#else
    ...
    deviceMipmapImage.fill(img_hmh, minDownscale, maxDownscale, deviceID);
}

void DeviceCache::addCameraParams(int camId, int downscale, const mvsUtils::MultiViewParams& mp, uint64_t deviceID)
{
    // get current device cache
    SingleDeviceCache& currentDeviceCache = getCurrentDeviceCache(deviceID);
    ...
    DeviceCameraParams cameraParameters_h = DeviceCameraParams();
    ...
    fillDeviceCameraParameters(currentDeviceCache.camParamMng, cameraParameters_h, deviceCameraParamsId);
}

const DeviceMipmapImage& DeviceCache::requestMipmapImage(int camId, const mvsUtils::MultiViewParams& mp, uint64_t deviceID)
{
    // get current device cache
    SingleDeviceCache& currentDeviceCache = getCurrentDeviceCache(deviceID);
...
const int DeviceCache::requestCameraParamsId(int camId, int downscale, const mvsUtils::MultiViewParams& mp, uint64_t deviceID)
{
    // get current device cache
    SingleDeviceCache& currentDeviceCache = getCurrentDeviceCache(deviceID);
...
MTL::Buffer* DeviceCache::requestGaussianArrayBuffer(uint64_t deviceID)
{
    // get current device cache
    SingleDeviceCache& currentDeviceCache = getCurrentDeviceCache(deviceID);
    return currentDeviceCache.gaussMng.getGaussianArrayBuffer();
}

MTL::Buffer* DeviceCache::requestGaussianOffsetBuffer(uint64_t deviceID)
{
    // get current device cache
    SingleDeviceCache& currentDeviceCache = getCurrentDeviceCache(deviceID);
    return currentDeviceCache.gaussMng.getOffsetBuffer();
}

MTL::Buffer* DeviceCache::requestDeviceCameraParamsBuffer(uint64_t deviceID)
{
    // get current device cache
    SingleDeviceCache& currentDeviceCache = getCurrentDeviceCache(deviceID);
    return currentDeviceCache.camParamMng.getCameraParamsBuffer();
}
