...
CustomPatchPattern& CustomPatchPattern::getInstance()
{
    if (_instance == nullptr)
        _instance = new CustomPatchPattern();
    return *_instance;
}

void CustomPatchPattern::addPatchPattern(const DevicePatchPattern& pattern, uint64_t deviceID)
{
    this->_devicePatchPatterns.insert_or_assign(deviceID, pattern);
}

std::optional<const DevicePatchPattern> CustomPatchPattern::getDevicePatchPattern(uint64_t deviceID) const
{
    if(this->_devicePatchPatterns.contains(deviceID))
        return this->_devicePatchPatterns.at(deviceID);
    return std::nullopt;
}

CustomPatchPattern* CustomPatchPattern::_instance = nullptr;

void buildCustomPatchPattern(const CustomPatchPatternParams& patchParams, const uint64_t deviceID)
{
    ...
    DevicePatchPattern patchPattern_h = DevicePatchPattern();
    ...
    // Add custom patch pattern
    CustomPatchPattern::getInstance().addPatchPattern(patchPattern_h, deviceID);
