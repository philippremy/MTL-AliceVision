...
void copyFloat2Map(image::Image<float>& out_mapX,
                   ...
                   const MTLHostMemoryHeap<float2, 2>& in_map_hmh,
                   const ROI& roi,
                   int downscale)
{
...
void copyFloat2Map(image::Image<float>& out_mapX,
                   image::Image<float>& out_mapY,
                   const MTLDeviceMemoryPitched<float2, 2>& in_map_dmp,
                   const ROI& roi,
                   int downscale)
{
    // copy float2 map from device pitched memory to host memory
    MTLHostMemoryHeap<float2, 2> map_hmh(in_map_dmp.getSize());
                    ...
                    const std::string& name)
{
    ...
    MTLHostMemoryHeap<float3, 2> map_hmh(in_map_dmp.getSize());
    map_hmh.copyFrom(in_map_dmp);
...
void writeDeviceImage(const MTLDeviceMemoryPitched<MTLRGBA, 2>& in_img_dmp, const std::string& path)
{
    const MTLSize<2>& imgSize = in_img_dmp.getSize();
    ...
    for (size_t x = 0; x < imgSize.x(); ++x)
    {
        ...
        {
            const MTLRGBA& rgba_hmh = img_hmh(x, y);
...
void writeNormalMap(int rc,
                    ...
                    const mvsUtils::TileParams& tileParams,
                    const ROI& roi,
                    const MTLDeviceMemoryPitched<float3, 2>& in_normalMap_dmp,
                    int scale,
                    int step,
                    const std::string& name)
{
...
void writeDepthThicknessMap(int rc,
                            ...
                            const mvsUtils::TileParams& tileParams,
                            const ROI& roi,
                            const MTLDeviceMemoryPitched<float2, 2>& in_depthThicknessMap_dmp,
                            int scale,
                            int step,
                            const std::string& name)
{
...
void writeDepthPixSizeMap(int rc,
                          ...
                          const mvsUtils::TileParams& tileParams,
                          const ROI& roi,
                          const MTLDeviceMemoryPitched<float2, 2>& in_depthPixSize_dmp,
                          int scale,
                          int step,
                          const std::string& name)
{
...
void writeDepthSimMapFromTileList(int rc,
                                  ...
                                  const std::vector<MTLHostMemoryHeap<float2, 2>>& in_depthSimMapTiles_hmh,
                                  int scale,
                                  int step,
                                  const std::string& name)
{
...
void resetDepthSimMap(MTLHostMemoryHeap<float2, 2>& inout_depthSimMap_hmh, float depth, float sim)
{
    const MTLSize<2>& depthSimMapSize = inout_depthSimMap_hmh.getSize();
