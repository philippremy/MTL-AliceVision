                                ...
                                const ROI& roi)
{
    ...
    for (int iy = 0; iy < sampleSize; ++iy)
    {
        for (int ix = 0; ix < sampleSize; ++ix)
        {
            ...
            {
                const float simValue = float(*get3DBufferAt_h<TSim>(in_volumeSim_hmh.getBuffer(), spitch, pitch, x, y, iz, MetalAddressSpaceTag<MetalAddressSpace::Device>{}));
                                ...
                                const ROI& roi)
{
    ...
    for (int iy = 0; iy < sampleSize; ++iy)
    {
        for (int ix = 0; ix < sampleSize; ++ix)
        {
            ...
            {
                const float simValue = float(*get3DBufferAt_h<TSimRefine>(in_volumeSim_hmh.getBuffer(), spitch, pitch, x, y, iz, MetalAddressSpaceTag<MetalAddressSpace::Device>{}));
                pDepths.push_back(simValue);
            }
        }
    }
    ...
    for (int vy = 0; vy < volDim[1]; vy += xyStep)
    {
        for (int vx = 0; vx < volDim[0]; vx += xyStep)
        {
            ...
            for (int vz = 0; vz < in_depths.size(); ++vz)
            {
                ...
                float simValue = *get3DBufferAt_h<TSim>(in_volumeSim_hmh.getBuffer(), spitch, pitch, vx, vy, vz, MetalAddressSpaceTag<MetalAddressSpace::Device>{});
                                 ...
                                 const ROI& roi)
{
    ...
    for (int vy = 0; vy < volDim[1]; ++vy)
    {
        ...
        for (int vx = xIdxStart; vx < xIdxStop; ++vx)
        {
            ...
            {
                const float simValue = float(*get3DBufferAt_h<TSimRefine>(in_volumeSim_hmh.getBuffer(), spitch, pitch, vx, vy, vz, MetalAddressSpaceTag<MetalAddressSpace::Device>{}));
                                          ...
                                          const ROI& roi)
{
    ...
    for (size_t vx = 0; vx < volDim.x(); ++vx)
    {
        ...
        {
            const float simValue = float(*get3DBufferAt_h<TSim>(in_volumeSim_hmh.getBuffer(), spitch, pitch, vx, vy, vz, MetalAddressSpaceTag<MetalAddressSpace::Device>{}));
    ...
    for (int vy = 0; vy < volDim[1]; vy += xyStep)
    {
        for (int vx = 0; vx < volDim[0]; vx += xyStep)
        {
            ...
            for (int vz = 0; vz < nbDepths; ++vz)
            {
                ...
                float4 colorValue = *get3DBufferAt_h<float4>(in_volumeSim_hmh.getBuffer(), spitch, pitch, vx, vy, vz, MetalAddressSpaceTag<MetalAddressSpace::Device>{});
