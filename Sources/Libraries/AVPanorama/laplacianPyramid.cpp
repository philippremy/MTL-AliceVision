...
#include <AVPanorama/laplacianPyramid.hpp>

#include <AVPanorama/feathering.hpp>
#include <AVPanorama/gaussian.hpp>
#include <AVPanorama/compositer.hpp>
                             ...
                             const BoundingBox& contentBoudingBox)
{
    ...
    for (int i = 0; i < source.height(); i++)
    {
        ...
        // aliceVision::image::Rgb<float> is non-trivially copyable, so we should assign instead of memcpy
        //memcpy((void*)&currentColor(di, contentBoudingBox.left), &source(i, 0), sizeof(image::RGBfColor) * source.width());
        for (std::size_t j = 0; j < source.width(); ++j)
        {
            currentColor(di, contentBoudingBox.left + j) = source(i, j);
        }
