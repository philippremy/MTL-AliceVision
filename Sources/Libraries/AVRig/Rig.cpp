...
#include <AVRig/Rig.hpp>
#include <AVRig/ResidualError.hpp>
...
void Rig::displayRelativePoseReprojection([[maybe_unused]] const geometry::Pose3& relativePose, [[maybe_unused]] std::size_t iLocalizer)
{
    ...
    for (int iView = 0; iView < witnessLocalizerResults.size(); ++iView)
    {
        ...
        {
            const std::size_t width = witnessLocalizerResults[iView].getIntrinsics().w();
            const std::size_t height = witnessLocalizerResults[iView].getIntrinsics().h();
            ...
            for (const IndexT iInlier : witnessLocalizerResults[iView].getInliers())
            {
                ...
                const Vec4 point3DH(point3D[0], point3D[1], point3D[2], 1.0);
                // Its reprojection
                Vec2 itsReprojection = witnessLocalizerResults[iView].getIntrinsics().transformProject(poseWitnessCamera, point3DH);
                // Its associated observation location
                const Vec2& point2D = points2D.col(iInlier);

                // Display reprojections and observations
                cv::rectangle(imgRes,
                              cv::Point(marginUndisto + point2D(0) - semiWidth, marginUndisto + point2D(1) - semiWidth),
                              cv::Point(marginUndisto + point2D(0) + semiWidth, marginUndisto + point2D(1) + semiWidth),
                              cv::Scalar(0, 255, 0));

                cv::rectangle(imgRes,
                              cv::Point(marginUndisto + itsReprojection(0) - semiWidth, marginUndisto + itsReprojection(1) - semiWidth),
                              cv::Point(marginUndisto + itsReprojection(0) + semiWidth, marginUndisto + itsReprojection(1) + semiWidth),
                              cv::Scalar(255, 0, 0));
            }
            cv::imshow("Reprojection", imgRes);
