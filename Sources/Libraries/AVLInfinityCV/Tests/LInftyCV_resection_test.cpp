...
BOOST_AUTO_TEST_CASE(Resection_L_Infinity_OSICLP)
{
    ...
    // Solve the problem and check that fitted value are good enough
    {
        ...
        BOOST_CHECK((BisectionLP<Resection_L1_ConstraintBuilder, LPConstraintsSparse>(wrapperOSICLPSolver, cstBuilder, &vec_solution, 1.0, 0.0)));

        // Move computed value to dataset for residual estimation.
        Mat34 P;
        P << vec_solution[0], vec_solution[1], vec_solution[2], vec_solution[3], vec_solution[4], vec_solution[5], vec_solution[6], vec_solution[7],
...
BOOST_AUTO_TEST_CASE(Resection_L_Infinity_MOSEK)
{
    ...
    // Solve the problem and check that fitted value are good enough
    {
        ...
        Resection_L1_ConstraintBuilder cstBuilder(d2._x[nResectionCameraIndex], XPoints);
        BOOST_CHECK((BisectionLP<Resection_L1_ConstraintBuilder, LPConstraintsSparse>(wrapperMosek, cstBuilder, &vec_solution, 1.0, 0.0)));
