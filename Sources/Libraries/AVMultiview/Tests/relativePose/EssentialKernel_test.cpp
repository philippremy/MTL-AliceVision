...
BOOST_AUTO_TEST_CASE(Essential8PSolver_IdFocal)
{
    ...
    for (int i = 0; i < iNviews; ++i)
    {
        ...
        for (int s = 0; s < Es.size(); ++s)
        {
            ...
            BOOST_CHECK(motionFromEssentialAndCorrespondence(Es.at(s).getMatrix(), d._K[i], x1Col, d._K[(i + 1) % iNviews], x2Col, &Rs[s], &ts[s]));
        }
...
BOOST_AUTO_TEST_CASE(Essential8PKernel_EightPointsRelativePose)
{
    ...
    NViewDataSet d = NRealisticCamerasRing(
      iNviews,
      Kernel::SolverT().getMinimumNbRequiredSamples(),
...
{
    Mat x1(2, 5), x2(2, 5);
    x1 << 0, 0, 0, .8, .8, 0, -.5, .8, 0, .8;
    x2 << 0, 0, 0, .8, .8, .1, -.4, .9, .1, .9;  // Y Translated camera.
