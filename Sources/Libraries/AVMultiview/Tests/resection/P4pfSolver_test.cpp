...
#include <AVMultiview/resection/ResectionResectionKernel.hpp>
#include <AVMultiview/resection/P4PfSolver.hpp>
#include <AVMultiviewTestData/NViewDataSet.hpp>
...
{
    double eps = 1e-3;
    return ((first._R - second._R).norm() < first._R.maxCoeff() * eps && (first._t - second._t).norm() < first._t.maxCoeff() * eps &&
...
BOOST_AUTO_TEST_CASE(Resection_P4Pf_AssignmentWithOneResult)
{
    // INPUT DATA
    Mat pt2D_1 = Mat(2, 4);
    pt2D_1 << -493.1500, 1051.9100, 176.9500, -1621.9800, -878.4550, -984.7530, -381.4300, -543.3450;
    Mat pt3D_1 = Mat(3, 4);
    pt3D_1 << 2.7518, 2.2375, 1.1940, 2.5778, 0.1336, -0.3709, 0.2048, -0.9147, -0.5491, -2.0511, 1.1480, -1.6151;
...
BOOST_AUTO_TEST_CASE(Resection_P4Pf_AssignmentWithMoreResults)
{
    // DATA
    Mat pt2D_2 = Mat(2, 4);
    pt2D_2 << 774.88000, -772.31000, -1661.63300, -1836.57300, -534.74500, -554.09400, -585.53300, -430.03000;
    Mat pt3D_2 = Mat(3, 4);
    pt3D_2 << 2.01852, 1.00709, 0.74051, 0.61962, 0.02133, 0.30770, 0.16656, 0.11249, -1.68077, 0.81502, 1.21056, 1.22624;
    ...
    bool pass = true;
    if (!(models_2.size() == 3 && isEqual(models_2.at(0), sol_21) && isEqual(models_2.at(1), sol_22) && isEqual(models_2.at(2), sol_23)))
        pass = false;
    BOOST_CHECK(pass);
}
...
BOOST_AUTO_TEST_CASE(Resection_P4Pf_AssignmentWithNoResults)
{
    // DATA
    Mat pt2D_3 = Mat(2, 4);
    pt2D_3 << 774.88000, -570.41000, -1881.86960, 1529.54000, -534.74500, -834.63100, -167.32000, -1203.28000;
    Mat pt3D_3 = Mat(3, 4);
    pt3D_3 << 2.01852, 1.28149, 0.55264, 2.29633, 0.02133, 0.26101, 0.14578, -1.80998, -1.68077, 0.70813, 1.22217, -1.76850;
