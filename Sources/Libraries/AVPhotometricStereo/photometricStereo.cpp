    ...
    for (int currentMaskIndex = 0; currentMaskIndex < numberOfMasks; ++currentMaskIndex)
    {
        ...
        if (PSParameters.isRobust)
        {
            ...
            for (size_t k = 0; k < max_iterations; ++k)
            {
                ...
                M_channel = Eigen::BDCSVD<Eigen::MatrixXf, Eigen::ComputeThinU | Eigen::ComputeThinV>(lightMat).solve(newImMat);
        ...
        {
            // Normal estimation
            M_channel = Eigen::BDCSVD<Eigen::MatrixXf, Eigen::ComputeThinU | Eigen::ComputeThinV>(lightMat).solve(imMat_gray);
            ...
            for (size_t ch = 0; ch < 3; ++ch)
            {
                ...
                M_channel = Eigen::BDCSVD<Eigen::MatrixXf, Eigen::ComputeThinU | Eigen::ComputeThinV>(lightMat).solve(pixelValues_channel);
