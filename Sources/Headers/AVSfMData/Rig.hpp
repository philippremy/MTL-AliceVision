...
inline std::string ERigSubPoseStatus_enumToString(ERigSubPoseStatus rigSubPoseStatus)
{
    ...
    {
        case ERigSubPoseStatus::UNINITIALIZED:
            return "uninitialized";
        case ERigSubPoseStatus::ESTIMATED:
            return "estimated";
        case ERigSubPoseStatus::CONSTANT:
            return "constant";
    }
...
inline ERigSubPoseStatus ERigSubPoseStatus_stringToEnum(const std::string& rigSubPoseStatus)
{
    ...
    if (status == "uninitialized")
        return ERigSubPoseStatus::UNINITIALIZED;
    if (status == "estimated")
        return ERigSubPoseStatus::ESTIMATED;
    if (status == "constant")
        return ERigSubPoseStatus::CONSTANT;
...
struct RigSubPose
{
     ...
     */
    bool operator==(const RigSubPose& other) const { return (status == other.status && pose == other.pose); }
};
...
class Rig
{
  public:
     ...
     */
    explicit Rig(unsigned int nbSubPoses = 0) { _subPoses.resize(nbSubPoses); }
     ...
     */
    bool operator==(const Rig& other) const { return _subPoses == other._subPoses; }
     ...
     */
    std::size_t getNbSubPoses() const { return _subPoses.size(); }

    /**
     * @brief Get the sub-poses const vector
     * @return rig sub-poses
     */
    const std::vector<RigSubPose>& getSubPoses() const { return _subPoses; }

    /**
     * @brief Get the sub-poses const vector
     * @return rig sub-poses
     */
    std::vector<RigSubPose>& getSubPoses() { return _subPoses; }
    /**
     * @brief Get the sub-pose for the given sub-pose index
     * @param index The sub-pose index
     * @return corresponding rig sub-pose
     */
    const RigSubPose& getSubPose(IndexT index) const { return _subPoses.at(index); }
     ...
     * @return corresponding rig sub-pose
     */
    RigSubPose& getSubPose(IndexT index) { return _subPoses.at(index); }
