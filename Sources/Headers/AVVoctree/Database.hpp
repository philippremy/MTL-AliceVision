...
#include <AVVoctree/VocabularyTree.hpp>
...
struct DocMatch
{
    ...
    bool operator<(const DocMatch& other) const { return score < other.score; }

    bool operator==(const DocMatch& other) const { return id == other.id && score == other.score; }
    bool operator!=(const DocMatch& other) const { return !(*this == other); }
};
...
class Database
{
  public:
     ...
     * @param[out] matches  IDs and scores for the top N matching database documents.
     */
    void find(const std::vector<Word>& document,
              std::size_t N,
              std::vector<DocMatch>& matches,
              const std::string& distanceMethod = "strongCommonPoints") const;
     ...
     * @param[out] matches  IDs and scores for the top N matching database documents.
     */
    void find(const SparseHistogram& query,
              std::size_t N,
              std::vector<DocMatch>& matches,
              const std::string& distanceMethod = "strongCommonPoints") const;
    ...
    const SparseHistogramPerImage& getSparseHistogramPerImage() const { return database_; }
