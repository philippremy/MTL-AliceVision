...
template<typename SOLVERINTERFACE>
OSIXSolver<SOLVERINTERFACE>::OSIXSolver(int nbParams)
  : ISolver(nbParams)
{
...
#if ALICEVISION_IS_DEFINED(ALICEVISION_HAVE_MOSEK)
template<>
OSIXSolver<OsiMskSolverInterface>::OSIXSolver(int nbParams)
  : ISolver(nbParams)
{
...
bool OSIXSolver<SOLVERINTERFACE>::setup(const LPConstraints& cstraints)  // cstraints <-> constraints
{
    ...
    const size_t nbLine = A.rows() + std::count(cstraints._vec_sign.begin(), cstraints._vec_sign.end(), LPConstraints::LP_EQUAL);
...
bool OSIXSolver<SOLVERINTERFACE>::setup(const LPConstraintsSparse& cstraints)  // cstraints <-> constraints
{
    ...
    for (int i = 0; i < A.rows(); ++i)
    {
        ...
        if (cstraints._vec_sign[i] == LPConstraints::LP_EQUAL || cstraints._vec_sign[i] == LPConstraints::LP_LESS_OR_EQUAL)
        {
            ...
            matrix->appendRow(vec_colno.size(), &vec_colno[0], &vec_value[0]);
            rowindex++;
        }
        ...
        {
            int coef = -1;
            for (std::vector<double>::iterator iter_val = vec_value.begin(); iter_val != vec_value.end(); iter_val++)
            {
                *iter_val *= coef;
            }
            row_lb[rowindex] = -1.0 * si->getInfinity();
...
// We need to make the following dependent on the SOLVERINTERFACE, because MOSEK does not support setting the pertubation. This is Clp only!
// We deal with this by checking with a constexpr whether the method exists in the template interface class.
template<typename T>
auto has_getModelPtr_impl(int) -> decltype(std::declval<T>().getModelPtr(), std::true_type{});

template<typename T>
std::false_type has_getModelPtr_impl(...);

template<typename T>
using has_getModelPtr = decltype(has_getModelPtr_impl<T>(0));
...
bool OSIXSolver<SOLVERINTERFACE>::solve()
{
    if (si != nullptr)
    {
        // Only apply this for the Clp based solver
        if constexpr (has_getModelPtr<SOLVERINTERFACE>::value)
        {
