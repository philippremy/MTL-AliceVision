...
#include <AVRobustEstimation/conditioning.hpp>
#include <AVRobustEstimation/ISolver.hpp>
#include <AVRobustEstimation/IRansacKernel.hpp>
#include <AVRobustEstimation/PointFittingKernel.hpp>
...
class PointFittingRansacKernel : public robustEstimation::IRansacKernel<ModelT_>,
                                 public robustEstimation::PointFittingKernel<SolverT_, ErrorT_, ModelT_>
{
  public:
     ...
     */
    std::size_t getMinimumNbRequiredSamples() const override { return PFKernel::getMinimumNbRequiredSamples(); }

    std::size_t getMinimumNbRequiredSamplesLS() const override { return _solverLs.getMinimumNbRequiredSamples(); }

    /**
     * @brief Return the maximum number of models for the solver
     * @return maximum number of models
     */
    std::size_t getMaximumNbModels() const override { return PFKernel::getMaximumNbModels(); }
     ...
     */
    void fit(const std::vector<std::size_t>& samples, std::vector<ModelT_>& models) const override { PFKernel::fit(samples, models); }
     ...
     */
    double error(std::size_t sample, const ModelT_& model) const override { return PFKernel::error(sample, model); }
     ...
     */
    void errors(const ModelT_& model, std::vector<double>& errors) const override { PFKernel::errors(model, errors); }

    /**
     * @brief Function used to unnormalize the model.
     * @param[in,out] model The model to unnormalize.
     */
    virtual void unnormalize(ModelT_& model) const override = 0;

    /**
     * @brief The number of elements in the data.
     * @return the number of elements in the data.
     */
    std::size_t nbSamples() const override { return PFKernel::nbSamples(); }

    /**
     * @brief Get logalpha0, Alpha0 is used to make the error adaptive to the image size
     * @return logalpha0
     */
    virtual double logalpha0() const override = 0;

    virtual double errorVectorDimension() const override = 0;
    virtual double unormalizeError(double val) const override = 0;
    virtual Mat3 normalizer1() const override = 0;
    virtual double thresholdNormalizer() const override = 0;
