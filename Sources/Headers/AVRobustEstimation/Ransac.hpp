...
#include <AVRobustEstimation/randSampling.hpp>
#include <AVRobustEstimation/ransacTools.hpp>
#include <limits>
#include <numeric>
#include <vector>
#include <iostream>
...
typename Kernel::ModelT RANSAC(const Kernel& kernel,
                               const Scorer& scorer,
                               ...
                               double outliers_probability = 1e-2)
{
    ...
    for (iteration = 0; iteration < max_iterations && iteration < really_max_iterations; ++iteration)
    {
        ...
        for (size_t i = 0; i < models.size(); ++i)
        {
            ...
            if (best_num_inliers < inliers.size())
            {
                ...
                if (bVerbose)
                {
                    ALICEVISION_LOG_DEBUG("inliers=" << best_num_inliers << "/" << total_samples << " (iter=" << iteration << ", sample=" << sample
                                                     << ")");
                }
                if (best_inlier_ratio)
                {
                    max_iterations = iterationsRequired(min_samples, outliers_probability, best_inlier_ratio);
                    if (bVerbose)
