...
#include <AVDepthMap/Metal/SimStat.metal>
...
inline void rotPointAroundVect(thread float3& out, thread float3& X, thread float3& vect, int angle)
{
    ...
    wy = w * y;
    wz = w * z;
    sa = sin(angle * (M_PI_F / 180.0f));
    ca = cos(angle * (M_PI_F / 180.0f));
...
inline float computePixSize(constant DeviceCameraParams& deviceCamParams, thread float3& p)
{
...
inline void getPixelFor3DPoint(thread float2& out, constant DeviceCameraParams& deviceCamParams, thread float3& X)
{
...
inline float3 get3DPointForPixelAndDepthFromRC(constant DeviceCameraParams& deviceCamParams, thread const float2& pix, const float depth)
{
...
inline float3 triangulateMatchRef(constant DeviceCameraParams& rcDeviceCamParams,
                                  constant DeviceCameraParams& tcDeviceCamParams,
                                  thread float2& refpix,
                                  thread float2& tarpix)
{
 ...
 */
inline float refineDepthSubPixel(thread float3& depths, thread float3& sims)
{
...
inline float compNCCby3DptsYK(constant DeviceCameraParams& rcDeviceCamParams,
                              constant DeviceCameraParams& tcDeviceCamParams,
                              texture2d<MTLPixelBaseType, access::sample> rcMipmapImage_tex,
                              texture2d<MTLPixelBaseType, access::sample> tcMipmapImage_tex,
                              sampler rcMipmapImage_samp,
                              sampler tcMipmapImage_samp,
                              ...
                              thread const Patch& patch)
{
    ...
    const MTLRGBA rcCenterColor = rcMipmapImage_tex.sample(rcMipmapImage_samp, float2((rp.x + 0.5f) * rcInvLevelWidth, (rp.y + 0.5f) * rcInvLevelHeight), level(rcMipmapLevel));
    const MTLRGBA tcCenterColor = tcMipmapImage_tex.sample(tcMipmapImage_samp, float2((tp.x + 0.5f) * tcInvLevelWidth, (tp.y + 0.5f) * tcInvLevelHeight), level(tcMipmapLevel));
    ...
    for(int yp = -wsh; yp <= wsh; ++yp)
    {
        for(int xp = -wsh; xp <= wsh; ++xp)
        {
            ...
            const MTLRGBA rcPatchCoordColor = rcMipmapImage_tex.sample(rcMipmapImage_samp, float2((rpc.x + 0.5f) * rcInvLevelWidth, (rpc.y + 0.5f) * rcInvLevelHeight), level(rcMipmapLevel));
            const MTLRGBA tcPatchCoordColor = tcMipmapImage_tex.sample(tcMipmapImage_samp, float2((tpc.x + 0.5f) * tcInvLevelWidth, (tpc.y + 0.5f) * tcInvLevelHeight), level(tcMipmapLevel));
...
inline float compNCCby3DptsYK_customPatchPattern(constant DeviceCameraParams& rcDeviceCamParams,
                                                 constant DeviceCameraParams& tcDeviceCamParams,
                                                 constant DevicePatchPattern& constantPatchPattern_d,
                                                 texture2d<MTLPixelBaseType, access::sample> rcMipmapImage_tex,
                                                 texture2d<MTLPixelBaseType, access::sample> tcMipmapImage_tex,
                                                 sampler rcMipmapImage_samp,
                                                 sampler tcMipmapImage_samp,
                                                 ...
                                                 thread Patch& patch)
{
    ...
    const float rcAlpha = rcMipmapImage_tex.sample(rcMipmapImage_samp, float2((rp.x + 0.5f) * rcInvLevelWidth, (rp.y + 0.5f) * rcInvLevelHeight), level(mipmapLevel)).w; // alpha only
    const float tcAlpha = tcMipmapImage_tex.sample(tcMipmapImage_samp, float2((tp.x + 0.5f) * tcInvLevelWidth, (tp.y + 0.5f) * tcInvLevelHeight), level(mipmapLevel)).w; // alpha only
    ...
    for(int s = 0; s < constantPatchPattern_d.nbSubparts; ++s)
    {
        ...
        constant DevicePatchPatternSubpart& subpart = constantPatchPattern_d.subparts[s];

        // compute patch center color (CIELAB) at subpart level resolution
        const MTLRGBA rcCenterColor = rcMipmapImage_tex.sample(rcMipmapImage_samp, float2((rp.x + 0.5f) * rcInvLevelWidth, (rp.y + 0.5f) * rcInvLevelHeight), level(rcMipmapLevel + subpart.level));
        const MTLRGBA tcCenterColor = tcMipmapImage_tex.sample(tcMipmapImage_samp, float2((tp.x + 0.5f) * tcInvLevelWidth, (tp.y + 0.5f) * tcInvLevelHeight), level(tcMipmapLevel + subpart.level));
        ...
        if(subpart.isCircle)
        {
            for(int c = 0; c < subpart.nbCoordinates; ++c)
            {
                ...
                constant float2& relativeCoord = subpart.coordinates[c];
                ...
                // get R and T image color (CIELAB) from 2d coordinates
                const MTLRGBA rcPatchCoordColor = rcMipmapImage_tex.sample(rcMipmapImage_samp, float2((rpc.x + 0.5f) * rcInvLevelWidth, (rpc.y + 0.5f) * rcInvLevelHeight), level(rcMipmapLevel + subpart.level));
                const MTLRGBA tcPatchCoordColor = tcMipmapImage_tex.sample(tcMipmapImage_samp, float2((tpc.x + 0.5f) * tcInvLevelWidth, (tpc.y + 0.5f) * tcInvLevelHeight), level(tcMipmapLevel + subpart.level));
