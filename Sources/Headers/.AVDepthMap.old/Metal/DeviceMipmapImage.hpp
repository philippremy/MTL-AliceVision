...
#ifndef DEVICEMIPMAPIMAGE_HPP
#define DEVICEMIPMAPIMAGE_HPP
...
class DeviceMipmapImage
{
  public:
     ...
     */
    void fill(uint64_t deviceID, const MTLSharedResource<MTLTexture, MTLRGBA>& in_img_hmh, int minDownscale, int maxDownscale);
     ...
     */
    const MTL::Size getDimensions(unsigned int downscale) const;
    ...
    const MTLSharedResource<MTLTexture, MTLRGBA>& getTexture() const;

  private:
    // private members

    std::unique_ptr<MTLSharedResource<MTLTexture, MTLRGBA>> _metalTexture = nullptr; //< the metal texture in device memory
