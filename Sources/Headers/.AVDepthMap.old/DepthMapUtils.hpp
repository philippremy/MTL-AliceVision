 ...
 * @param[in] path the path of the output image on disk
 */
void writeDeviceImage(const VulkanMemoryBase<VulkanRGBA>& in_img_dmp, const std::string& path);
 ...
 * @param[in] sim the sim reset value
 */
void resetDepthSimMap(const VulkanMemoryBase<float2>& inout_depthSimMap_hmh, float depth = -1.f, float sim = 1.f);
 ...
 * @param[in] sim the sim reset value
 */
void resetDepthSimMap(MTLSharedResource<MTLTexture, float4>& inout_depthSimMap_hmh, float depth = -1.f, float sim = 1.f);
