...
#define ALICEVISION_RUNTIME_ASSERT(BOOL, MSG) \
    if(!(BOOL)) \
        ALICEVISION_THROW_ERROR("Runtime Assertion failed in " << __FILE__ << " at " << __LINE__ << ", called from " << __FUNCTION__ << ": " << #BOOL << ". Message: " << MSG)
