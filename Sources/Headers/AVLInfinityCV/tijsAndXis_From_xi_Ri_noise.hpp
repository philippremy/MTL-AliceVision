...
#define OFSVAR(k, el) (offsetStart + 3 * (k) + (el))
    ...
    // LP_EQUAL
    A.coeffRef(rowPos, TVAR(0, 0)) = 1.0;
    C(rowPos) = 0.0;
    vec_sign[rowPos] = LPConstraints::LP_EQUAL;
    ++rowPos;
    A.coeffRef(rowPos, TVAR(0, 1)) = 1.0;
    C(rowPos) = 0.0;
    vec_sign[rowPos] = LPConstraints::LP_EQUAL;
    ++rowPos;
    A.coeffRef(rowPos, TVAR(0, 2)) = 1.0;
    C(rowPos) = 0.0;
    vec_sign[rowPos] = LPConstraints::LP_EQUAL;
    ++rowPos;
...
{
    TiXi_withNoise_L1_ConstraintBuilder(const std::vector<Mat3>& vec_Ri, const Mat& M)
    {
        _M = M;
        _vec_Ri = vec_Ri;
    }
