...
struct spvDescriptorSetBuffer0
{
    ...
    texture2d<float> in_depths_d [[id(1)]];
    texture2d<float> rcDeviceMipmapImage [[id(2)]];
    sampler rcDeviceMipmapImageSmplr [[id(3)]];
    texture2d<float> tcDeviceMipmapImage [[id(4)]];
    sampler tcDeviceMipmapImageSmplr [[id(5)]];
    constant DevicePatchPatternConstant* constantPatchPattern_d [[id(6)]];
    texture3d<uint, access::read_write> out_volume1st_d [[id(7)]];
    texture3d<uint, access::read_write> out_volume2nd_d [[id(8)]];
};
...
static inline __attribute__((always_inline))
float3 get3DPointForPixelAndFrontoParellePlaneRC(DeviceCameraParams deviceCamParams, float2 pix, thread const float& fpPlaneDepth)
{
...
static inline __attribute__((always_inline))
void computeRotCSEpip(thread Patch& pat, DeviceCameraParams rcDeviceCamParams, DeviceCameraParams tcDeviceCamParams)
{
...
static inline __attribute__((always_inline))
void volume_computePatch(thread Patch& pat, DeviceCameraParams rcDeviceCamParams, DeviceCameraParams tcDeviceCamParams, float fpPlaneDepth, float2 pix)
{
    float param = fpPlaneDepth;
    pat.p = get3DPointForPixelAndFrontoParellePlaneRC(rcDeviceCamParams, pix, param);
    pat.d = computePixSize(rcDeviceCamParams, pat.p);
    Patch param_1 = pat;
    computeRotCSEpip(param_1, rcDeviceCamParams, tcDeviceCamParams);
    pat = param_1;
}
...
float compNCCby3DptsYK_customPatchPattern(DeviceCameraParams rcDeviceCamParams, DeviceCameraParams tcDeviceCamParams, texture2d<float> rcMipmapImage_tex, sampler rcMipmapImage_texSmplr, texture2d<float> tcMipmapImage_tex, sampler tcMipmapImage_texSmplr, uint rcLevelWidth, uint rcLevelHeight, uint tcLevelWidth, uint tcLevelHeight, float mipmapLevel, float invGammaC, float invGammaP, bool useConsistentScale, Patch pat, bool TInvertAndFilter, DevicePatchPattern constantPatchPattern_d)
{
    ...
    if (!_1061)
    {
        _1071 = rp.x > (float(rcLevelWidth - 1u) - 2.0);
    }
    else
    {
        _1071 = _1061;
    ...
    if (!_1078)
    {
        _1088 = tp.x > (float(tcLevelWidth - 1u) - 2.0);
    }
    else
    {
        _1088 = _1078;
    ...
    if (!_1095)
    {
        _1105 = rp.y > (float(rcLevelHeight - 1u) - 2.0);
    }
    else
    {
        _1105 = _1095;
    ...
    if (!_1112)
    {
        _1122 = tp.y > (float(tcLevelHeight - 1u) - 2.0);
    }
    else
    {
        _1122 = _1112;
    ...
    for (int s = 0; s < constantPatchPattern_d.nbSubparts; s++)
    {
        ...
        {
            int _1348 = -subpart.wsh;
            for (int yp = _1348; yp <= subpart.wsh; yp++)
            {
                int _1361 = -subpart.wsh;
                for (int xp = _1361; xp <= subpart.wsh; xp++)
                {
        ...
        SimStat param_6 = sst;
        float _1466 = SimStat_computeWSim(param_6);
        sst = param_6;
        float fsimSubpart = _1466;
        if (fsimSubpart < 0.0)
        {
            if (TInvertAndFilter)
            {
...
float compNCCby3DptsYK(DeviceCameraParams rcDeviceCamParams, DeviceCameraParams tcDeviceCamParams, texture2d<float> rcMipmapImage_tex, sampler rcMipmapImage_texSmplr, texture2d<float> tcMipmapImage_tex, sampler tcMipmapImage_texSmplr, uint rcLevelWidth, uint rcLevelHeight, uint tcLevelWidth, uint tcLevelHeight, float mipmapLevel, int wsh, float invGammaC, float invGammaP, bool useConsistentScale, Patch pat, bool TInvertAndFilter)
{
    ...
    if (!_781)
    {
        _792 = rp.x > (float(rcLevelWidth - 1u) - dd);
    }
    else
    {
        _792 = _781;
    ...
    if (!_800)
    {
        _811 = tp.x > (float(tcLevelWidth - 1u) - dd);
    }
    else
    {
        _811 = _800;
    ...
    if (!_819)
    {
        _830 = rp.y > (float(rcLevelHeight - 1u) - dd);
    }
    else
    {
        _830 = _819;
    ...
    if (!_838)
    {
        _849 = tp.y > (float(tcLevelHeight - 1u) - dd);
    }
    else
    {
        _849 = _838;
    ...
    bool _917 = rcCenterColor.w < 229.5;
    bool _925;
    if (!_917)
    {
        _925 = tcCenterColor.w < 102.0;
    }
    else
    {
        _925 = _917;
    ...
    int _931 = -wsh;
    for (int yp = _931; yp <= wsh; yp++)
    {
        int _940 = -wsh;
        for (int xp = _940; xp <= wsh; xp++)
        {
    ...
    {
        SimStat param_4 = sst;
        float _1033 = SimStat_computeWSim(param_4);
        sst = param_4;
        float fsim = _1033;
        float param_5 = 0.0;
        float param_6 = 1.0;
    ...
    float _1047 = SimStat_computeWSim(param_10);
    sst = param_10;
    return _1047;
}
...
kernel void main0(constant spvDescriptorSetBuffer0& spvDescriptorSet0 [[buffer(0)]], constant PushConstants& pushConstants [[buffer(1)]], uint3 gl_GlobalInvocationID [[thread_position_in_grid]])
{
    ...
    bool _1583 = roiX >= ROI_width(param);
    bool _1605;
    if (!_1583)
    {
        ROI param_1;
    ...
    float depthPlane = spvDescriptorSet0.in_depths_d.read(uint2(int2(int(vz), 0))).x;
    Patch pat = Patch{ float3(0.0), float3(0.0), float3(0.0), float3(0.0), 0.0 };
    Patch param_2 = pat;
    volume_computePatch(param_2, rcDeviceCamParams, tcDeviceCamParams, depthPlane, float2(x, y));
    pat = param_2;
    float fsim = as_type<float>(0x7f800000u /* inf */);
    if (pushConstants.useCustomPatchPattern != 0u)
    {
        ...
        fsim = compNCCby3DptsYK_customPatchPattern(rcDeviceCamParams, tcDeviceCamParams, spvDescriptorSet0.rcDeviceMipmapImage, spvDescriptorSet0.rcDeviceMipmapImageSmplr, spvDescriptorSet0.tcDeviceMipmapImage, spvDescriptorSet0.tcDeviceMipmapImageSmplr, pushConstants.rcSgmLevelWidth, pushConstants.rcSgmLevelHeight, pushConstants.tcSgmLevelWidth, pushConstants.tcSgmLevelHeight, pushConstants.rcMipmapLevel, pushConstants.invGammaC, pushConstants.invGammaP, pushConstants.useConsistentScale != 0u, pat, false, arg);
    }
    else
    {
        fsim = compNCCby3DptsYK(rcDeviceCamParams, tcDeviceCamParams, spvDescriptorSet0.rcDeviceMipmapImage, spvDescriptorSet0.rcDeviceMipmapImageSmplr, spvDescriptorSet0.tcDeviceMipmapImage, spvDescriptorSet0.tcDeviceMipmapImageSmplr, pushConstants.rcSgmLevelWidth, pushConstants.rcSgmLevelHeight, pushConstants.tcSgmLevelWidth, pushConstants.tcSgmLevelHeight, pushConstants.rcMipmapLevel, pushConstants.wsh, pushConstants.invGammaC, pushConstants.invGammaP, pushConstants.useConsistentScale != 0u, pat, false);
    }
    if (isinf(fsim))
    {
        fsim = 255.0;
    ...
    {
        fsim = (fsim - (-1.0)) * 0.5;
        fsim = fast::min(1.0, fast::max(0.0, fsim));
        fsim *= 254.0;
    }
    spvDescriptorSet0.out_volume1st_d.fence();
    uint4 fsim_1st = spvDescriptorSet0.out_volume1st_d.read(uint3(int3(int(vx), int(vy), int(vz))));
    spvDescriptorSet0.out_volume2nd_d.fence();
    uint4 fsim_2nd = spvDescriptorSet0.out_volume2nd_d.read(uint3(int3(int(vx), int(vy), int(vz))));
    if (fsim < float(fsim_1st.x))
    {
        spvDescriptorSet0.out_volume2nd_d.write(uint4(fsim_1st.x, fsim_2nd.y, fsim_2nd.z, fsim_2nd.w), uint3(int3(int(vx), int(vy), int(vz))));
        spvDescriptorSet0.out_volume1st_d.write(uint4(uint(fsim), fsim_1st.y, fsim_1st.z, fsim_1st.w), uint3(int3(int(vx), int(vy), int(vz))));
    }
    else
    {
        if (fsim < float(fsim_2nd.x))
        {
            spvDescriptorSet0.out_volume2nd_d.write(uint4(uint(fsim), fsim_2nd.y, fsim_2nd.z, fsim_2nd.w), uint3(int3(int(vx), int(vy), int(vz))));
        }
    }
}
