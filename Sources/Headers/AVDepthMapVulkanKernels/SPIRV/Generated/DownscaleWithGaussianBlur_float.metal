    ...
    if (_71)
    {
        ...
        for (int i = _95; i <= pushConstants.gaussRadius; i++)
        {
            ...
            for (int j = _108; j <= pushConstants.gaussRadius; j++)
            {
                ...
                float4 curPix = spvDescriptorSet0.in_img_tex.read(uint2(texel), 0);
        ...
        float4 outPix = float4(0.0);
        outPix = accPix / float4(sumFactor);
        spvDescriptorSet0.out_downscaledImg_d.write(outPix, uint2(int2(int(x), int(y))));
    }
}
