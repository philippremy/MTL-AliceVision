...
struct spvDescriptorSetBuffer0
{
    ...
    texture2d<uint, access::write> out_mipmappedArrayLevel [[id(2)]];
    texture2d<uint> in_image [[id(3)]];
...
kernel void main0(constant spvDescriptorSetBuffer0& spvDescriptorSet0 [[buffer(0)]], constant PushConstants& pushConstants [[buffer(1)]], uint3 gl_GlobalInvocationID [[thread_position_in_grid]])
{
    ...
    for (int i = _101; i <= pushConstants.radius; i++)
    {
        ...
        for (int j = _114; j <= pushConstants.radius; j++)
        {
            ...
            uint4 color = spvDescriptorSet0.in_image.sample(spvDescriptorSet0.in_imageSmplr, uv, level(float(pushConstants.out_mipLevel - 1u)));
            sumColor += (float4(color) * factor);
            sumFactor += float(uint(factor));
        }
    }
    uint4 color_1 = uint4(fast::clamp(round(sumColor), float4(0.0), float4(255.0)));
