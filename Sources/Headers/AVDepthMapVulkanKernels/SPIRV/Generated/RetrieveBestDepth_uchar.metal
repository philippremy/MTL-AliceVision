...
struct spvDescriptorSetBuffer0
{
    ...
    texture3d<uint> in_volSim_d [[id(1)]];
...
kernel void main0(constant spvDescriptorSetBuffer0& spvDescriptorSet0 [[buffer(0)]], constant PushConstants& pushConstants [[buffer(1)]], uint3 gl_GlobalInvocationID [[thread_position_in_grid]])
{
    ...
    {
        float simAtZ = float(spvDescriptorSet0.in_volSim_d.read(uint3(int3(int(vx), int(vy), vz))).x);
        if (simAtZ < bestSim)
        {
            bestSim = simAtZ;
            bestZIdx = vz;
    ...
    bool _650 = bestZIdx == (-1);
    bool _659;
    if (!_650)
    {
        _659 = bestSim > pushConstants.maxSimilarity;
    }
    else
    {
        _659 = _650;
