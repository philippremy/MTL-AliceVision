...
struct spvDescriptorSetBuffer0
{
    ...
    texture3d<float, access::read_write> out_volume1st_d [[id(7)]];
    texture3d<float, access::read_write> out_volume2nd_d [[id(8)]];
};
...
kernel void main0(constant spvDescriptorSetBuffer0& spvDescriptorSet0 [[buffer(0)]], constant PushConstants& pushConstants [[buffer(1)]], uint3 gl_GlobalInvocationID [[thread_position_in_grid]])
{
    ...
    float4 fsim_1st = spvDescriptorSet0.out_volume1st_d.read(uint3(int3(int(vx), int(vy), int(vz))));
    spvDescriptorSet0.out_volume2nd_d.fence();
    float4 fsim_2nd = spvDescriptorSet0.out_volume2nd_d.read(uint3(int3(int(vx), int(vy), int(vz))));
    if (fsim < fsim_1st.x)
    {
        spvDescriptorSet0.out_volume2nd_d.write(float4(fsim_1st.x, fsim_2nd.y, fsim_2nd.z, fsim_2nd.w), uint3(int3(int(vx), int(vy), int(vz))));
        spvDescriptorSet0.out_volume1st_d.write(float4(fsim, fsim_1st.y, fsim_1st.z, fsim_1st.w), uint3(int3(int(vx), int(vy), int(vz))));
    }
    else
    {
        if (fsim < fsim_2nd.x)
        {
            spvDescriptorSet0.out_volume2nd_d.write(float4(fsim, fsim_2nd.y, fsim_2nd.z, fsim_2nd.w), uint3(int3(int(vx), int(vy), int(vz))));
        }
    }
}
