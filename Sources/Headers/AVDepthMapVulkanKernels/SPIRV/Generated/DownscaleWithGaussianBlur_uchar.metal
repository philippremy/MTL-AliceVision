...
struct spvDescriptorSetBuffer0
{
    ...
    texture2d<uint, access::write> out_downscaledImg_d [[id(2)]];
    texture2d<uint> in_img_tex [[id(3)]];
...
kernel void main0(constant spvDescriptorSetBuffer0& spvDescriptorSet0 [[buffer(0)]], constant PushConstants& pushConstants [[buffer(1)]], uint3 gl_GlobalInvocationID [[thread_position_in_grid]])
{
    ...
    if (_71)
    {
        ...
        float4 accPix = float4(0.0);
        float sumFactor = 0.0;
        int _95 = -pushConstants.gaussRadius;
        for (int i = _95; i <= pushConstants.gaussRadius; i++)
        {
            int _108 = -pushConstants.gaussRadius;
            for (int j = _108; j <= pushConstants.gaussRadius; j++)
            {
                ...
                uint4 curPixInt = spvDescriptorSet0.in_img_tex.read(uint2(texel), 0);
                float4 curPix = float4(curPixInt) / float4(255.0);
        ...
        float4 outPix = accPix / float4(sumFactor);
        uint4 outPixInt = uint4(fast::clamp(outPix * 255.0, float4(0.0), float4(255.0)));
        spvDescriptorSet0.out_downscaledImg_d.write(outPixInt, uint2(int2(int(x), int(y))));
    }
}
