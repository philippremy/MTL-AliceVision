...
{
    texture2d<float, access::write> out_upscaledMap_d [[id(0)]];
    texture2d<float> in_map_d [[id(1)]];
};
...
kernel void main0(constant spvDescriptorSetBuffer0& spvDescriptorSet0 [[buffer(0)]], constant PushConstants& pushConstants [[buffer(1)]], uint3 gl_GlobalInvocationID [[thread_position_in_grid]])
{
    ...
    float ox = (float(x) - 0.5) * pushConstants.ratio;
    float oy = (float(y) - 0.5) * pushConstants.ratio;
    ROI param_2;
    ...
    int xp = min(int(floor(ox + 0.5)), (int(float(ROI_width(param_2)) * pushConstants.ratio) - 1));
    ROI param_3;
    ...
    int yp = min(int(floor(oy + 0.5)), (int(float(ROI_height(param_3)) * pushConstants.ratio) - 1));
    spvDescriptorSet0.out_upscaledMap_d.write(spvDescriptorSet0.in_map_d.read(uint2(int2(xp, yp))), uint2(int2(int(x), int(y))));
}
