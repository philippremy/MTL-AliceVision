...
{
    texture2d<uint, access::read_write> inout_img_dmp [[id(0)]];
};
...
kernel void main0(constant spvDescriptorSetBuffer0& spvDescriptorSet0 [[buffer(0)]], uint3 gl_GlobalInvocationID [[thread_position_in_grid]])
{
    ...
    uint4 rgba = spvDescriptorSet0.inout_img_dmp.read(uint2(int2(int(x), int(y))));
    float3 flab = xyz2lab(rgb2xyz(float3(float(rgba.x) * 0.0039215688593685626983642578125, float(rgba.y) * 0.0039215688593685626983642578125, float(rgba.z) * 0.0039215688593685626983642578125)));
    spvDescriptorSet0.inout_img_dmp.write(uint4(uint3(fast::clamp(flab, float3(0.0), float3(255.0))), rgba.w), uint2(int2(int(x), int(y))));
}
