...
kernel void main0(constant spvDescriptorSetBuffer0& spvDescriptorSet0 [[buffer(0)]], constant PushConstants& pushConstants [[buffer(1)]], uint3 gl_GlobalInvocationID [[thread_position_in_grid]])
{
    ...
    half px = half(1.0) / half(outImageSize.x);
    half py = half(1.0) / half(outImageSize.y);
    half4 sumColor = half4(half(0.0));
    half sumFactor = half(0.0);
    int _102 = -pushConstants.radius;
    for (int i = _102; i <= pushConstants.radius; i++)
    {
        int _115 = -pushConstants.radius;
        for (int j = _115; j <= pushConstants.radius; j++)
        {
            int param = 1;
            ...
            float2 uv = float2((float(x + uint(j)) + 0.5) * float(px), (float(y + uint(i)) + 0.5) * float(py));
            half4 color = half4(spvDescriptorSet0.in_image.sample(spvDescriptorSet0.in_imageSmplr, uv, level(float(pushConstants.out_mipLevel - 1u))));
            sumColor += (color * half(factor));
            sumFactor += half(factor);
        }
    }
    half4 color_1 = sumColor / half4(sumFactor);
    spvDescriptorSet0.out_mipmappedArrayLevel.write(float4(color_1), uint2(int2(int(x), int(y))));
}
