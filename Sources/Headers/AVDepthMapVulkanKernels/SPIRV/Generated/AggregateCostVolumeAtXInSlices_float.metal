...
{
    texture2d<float, access::read_write> xzSliceForY_d [[id(0)]];
    texture2d<float> rcMipmapImage_tex [[id(1)]];
    sampler rcMipmapImage_texSmplr [[id(2)]];
    texture2d<float> bestSimInYm1_d [[id(3)]];
    texture2d<float> xzSliceForYm1_d [[id(4)]];
    texture3d<float, access::read_write> volAgr_d [[id(5)]];
};
...
kernel void main0(constant spvDescriptorSetBuffer0& spvDescriptorSet0 [[buffer(0)]], constant PushConstants& pushConstants [[buffer(1)]], uint3 gl_GlobalInvocationID [[thread_position_in_grid]])
{
    ...
    float4 sim_xz = spvDescriptorSet0.xzSliceForY_d.read(uint2(int2(int(x), int(z))));
    float pathCost = 255.0;
    bool _529 = z >= 1u;
    bool _538;
    if (_529)
    {
        _538 = z < uint(pushConstants.volDim[2u] - 1);
    }
    else
    {
        _538 = _529;
    ...
    if (_538)
    {
        ...
        float bestCostInColM1 = spvDescriptorSet0.bestSimInYm1_d.read(uint2(int2(int(x), 0))).x;
        float pathCostMDM1 = spvDescriptorSet0.xzSliceForYm1_d.read(uint2(int2(int(x), int(z - 1u)))).x;
        float pathCostMD = spvDescriptorSet0.xzSliceForYm1_d.read(uint2(int2(int(x), int(z)))).x;
        float pathCostMDP1 = spvDescriptorSet0.xzSliceForYm1_d.read(uint2(int2(int(x), int(z + 1u)))).x;
        float param_5 = pathCostMD;
        float param_6 = pathCostMDM1 + pushConstants.P1;
        float param_7 = pathCostMDP1 + pushConstants.P1;
        float param_8 = bestCostInColM1 + P2;
    ...
    float4 volume_xyz = spvDescriptorSet0.volAgr_d.read(uint3(v));
    float val = ((volume_xyz.x * float(pushConstants.filteringIndex)) + pathCost) / float(pushConstants.filteringIndex + 1);
    spvDescriptorSet0.volAgr_d.write(float4(val, volume_xyz.y, volume_xyz.z, volume_xyz.w), uint3(v));
}
