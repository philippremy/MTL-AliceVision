...
using CudaColorBaseType = _Float16;
    #else
...
#pragma unroll
    ...
    inline METAL_QUALIFIER size_t& operator[](size_t i) { return size[i]; }
    inline CudaSizeBase operator+(METAL_QUALIFIER const CudaSizeBase<Dim>& s) const
    {
        CudaSizeBase<Dim> r;
...
template<unsigned Dim>
bool operator==(METAL_QUALIFIER const CudaSizeBase<Dim>& s1, METAL_QUALIFIER const CudaSizeBase<Dim>& s2)
{
    for (int i = Dim; i--;)
        if (s1[i] != s2[i])
            return false;
...
template<unsigned Dim>
CudaSize<Dim> operator/(METAL_QUALIFIER const CudaSize<Dim>& lhs, METAL_QUALIFIER const float& rhs)
{
    if (rhs == 0)
...
template<unsigned Dim>
CudaSize<Dim> operator-(METAL_QUALIFIER const CudaSize<Dim>& lhs, METAL_QUALIFIER const CudaSize<Dim>& rhs)
{
    CudaSize<Dim> out = lhs;
    for (size_t i = Dim; i--;)
        out[i] -= rhs[i];
...
class CudaMemorySizeBase
{
  ...
  public:
    ...
    explicit CudaMemorySizeBase(METAL_QUALIFIER const CudaSize<Dim>& size)
      : _size(size),
     ...
     * parameter.
     */
    void setSize(METAL_QUALIFIER const CudaSize<Dim>& size, bool computePitch)
    {
        _size = size;
        if (computePitch)
        {
     ...
     */
    inline METAL_QUALIFIER const CudaSize<Dim>& getSize() const { return _size; }
  ...
  protected:
     ...
     */
    inline METAL_QUALIFIER size_t& getPitchRef() { return _pitch; }
};
...
class CudaHostMemoryHeap : public CudaMemorySizeBase<Type, Dim>
{
  ...
  public:
    ...
    #if !defined(__METAL__)
    explicit CudaHostMemoryHeap(METAL_QUALIFIER const CudaSize<Dim>& size)
      : buffer(nullptr)
    {
        allocate(size);
    }
    ...
    #if !defined(__METAL__)
    void copyFrom(const CudaDeviceMemoryPitched<Type, Dim>& src, uint64_t deviceID);
    #endif
  ...
  public:
    void allocate(const CudaSize<Dim>& size)
    {
        ...
        this->_mtlBuffer = MTL::CreateSystemDefaultDevice()->newBuffer(this->getBytesUnpadded(), MTL::ResourceStorageModeManaged);
        this->buffer = (Type*)this->_mtlBuffer->contents();

    }

    void deallocate()
    {
        if (buffer == nullptr && _mtlBuffer == nullptr)
            return;
        _mtlBuffer->release();
        buffer = nullptr;
    }
...
class CudaDeviceMemoryPitched : public CudaMemorySizeBase<Type, Dim>
{
  ...
  public:
    ...
    // see below with copy() functions
    void copyFrom(const CudaDeviceMemoryPitched<Type, Dim>& src, uint64_t deviceID);
    void copyFrom(const CudaHostMemoryHeap<Type, Dim>& src, uint64_t deviceID);
