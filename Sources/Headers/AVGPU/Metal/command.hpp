...
class MTLCommandManager
{
  ...
  public:
     ...
     * type.
     *
     * @warning If Push Constants are bound, buffer index 0 is reserved for Push Constants.
     *
    ...
    MTLCommandManager* bind(const MTLSharedResource<Resource, T>& resource, uint64_t idx);

    /**
     * @brief Creates a new sampler and binds it to the specified index
     *
     * This method creates a new sampler and binds it to the given index.
     * The settings of the sampler match the CUDA implementation of
     * AliceVision.
     *
     * @param idx The index at which the sampler should be bound to
     * @return A pointer to this (MTLCommandManager) for method chaining
     */
    MTLCommandManager* createSampler(uint64_t idx);
     ...
     * AliceVision.
     *
     * @param normalizedCoords Whether normalized coordinates (0.0-1.0) should be used
     * @param subpixelInterpolation Whether subpixels should be interpolated
     * @param idx The index at which the sampler should be bound to
     * @return A pointer to this (MTLCommandManager) for method chaining
     */
    MTLCommandManager* createSampler(bool normalizedCoords, bool subpixelInterpolation, uint64_t idx);

    /**
     * @brief Directly binds the provided push constants to buffer(0)
     *
     * This method binds "Push Constants" directly to GPU memory,
     * making them available at buffer binding 0.
     *
  ...
  private:
    explicit MTLCommandManager(NS::SharedPtr<MTL::Device> device);
...
#include <AVGPU/Metal/Impl/command.inl>
