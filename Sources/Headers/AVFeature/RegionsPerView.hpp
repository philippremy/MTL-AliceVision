...
class MapRegionsPerDesc : public std::map<feature::EImageDescriberType, std::unique_ptr<feature::Regions>>
{
  public:
    ...
    template<class T>
    T getRegions(feature::EImageDescriberType descType)
    {
        return dynamic_cast<T&>(*this->at(descType));
    }

    template<class T>
    const T getRegions(feature::EImageDescriberType descType) const
    {
        return dynamic_cast<const T&>(*this->at(descType));
    }
};
...
{
  public:
    MapRegionsPerView& getData() { return _data; }
    ...
    const feature::MapRegionsPerDesc& getRegionsPerDesc(IndexT viewId) const { return _data.at(viewId); }
    const feature::MapRegionsPerDesc& getDataPerDesc(IndexT viewId) const { return _data.at(viewId); }
    ...
    const MapRegionsPerDesc& getAllRegions(IndexT viewId) const { return _data.at(viewId); }
