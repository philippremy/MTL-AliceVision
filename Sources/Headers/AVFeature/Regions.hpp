...
#include <AVFeature/PointFeature.hpp>
#include <AVFeature/Descriptor.hpp>
#include <AVFeature/metric.hpp>
...
struct FeatureInImage
{
    ...
    bool operator<(const FeatureInImage& other) const { return _featureIndex < other._featureIndex; }
};
...
class Regions
{
  ...
  public:
    void LoadFeatures(const std::string& sfileNameFeats) { loadFeatsFromFile(sfileNameFeats, _vec_feats); }

    PointFeatures GetRegionsPositions() const { return PointFeatures(_vec_feats.begin(), _vec_feats.end()); }

    Vec2 GetRegionPosition(std::size_t i) const { return Vec2f(_vec_feats[i].coords()).cast<double>(); }
    ...
    virtual std::unique_ptr<Regions> createFilteredRegions(const std::vector<FeatureInImage>& featuresInImage,
...
class FeatDescRegions : public Regions
{
  ...
  public:
    ...
    Regions* EmptyClone() const override { return new This(); }
     ...
     */
    std::unique_ptr<Regions> createFilteredRegions(const std::vector<FeatureInImage>& featuresInImage,
                                                   std::vector<IndexT>& out_associated3dPoint,
