...
class DeviceMipmapImage
{
  public:
     ...
     */
    void fill(const MTLHostMemoryHeap<MTLRGBA, 2>& in_img_hmh, int minDownscale, int maxDownscale, uint64_t deviceID);
     ...
     */
    MTLSize<2> getDimensions(unsigned int downscale) const;
     ...
     */
    inline MTL::Buffer* getTextureObject() const { return _textureObject.getBuffer(); }

    inline uint64_t getLevel0Width() const { return _textureObject.getWidth(0); }
    inline uint64_t getLevel0Height() const { return _textureObject.getHeight(0); }
    inline uint64_t getLevelCount() const { return _levels; }

  private:
    // private members

    MTLMipmappedTexture<MTLRGBA> _textureObject;     //< mipmapped array texture object with normalized coordinates
