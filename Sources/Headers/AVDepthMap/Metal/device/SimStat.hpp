...
struct simStat
{
    ...
    float computeSimSub(constant simStat& ss)
    {
        // NCC
        float avx = ss.xsum / ss.count;
        float avy = ss.ysum / ss.count;
    ...
    */
    ...
    void update(device const float2& g)
    {
        count += 1.0f;
        xsum += g.x;
        ysum += g.y;
    ...
    void update(constant const float2& g)
    {
        count += 1.0f;
        xsum += g.x;
        ysum += g.y;
    ...
    void update(constant const float2& g, float w)
    {
        wsum += w;
        count += 1.0f;
        xsum += w * g.x;
    ...
    void update(constant const float3& c1, constant const float3& c2)
    {
        float2 g;
        g.x = c1.x;
        g.y = c2.x;
