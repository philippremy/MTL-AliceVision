...
{
    return make_float3(c.x <= 0.04045f ? c.x / 12.92f : pow((c.x + 0.055f) / 1.055f, 2.4f),
 ...
 * @return float3 HSL
 */
inline float3 rgb2hsl(device const float3& c)
{
    const float cmin = fmin(c.x, fmin(c.y, c.z));
    const float cmax = fmax(c.x, fmax(c.y, c.z));
 ...
 * @return float3 HSL
 */
inline float3 rgb2hsl(thread const float3& c)
{
    const float cmin = fmin(c.x, fmin(c.y, c.z));
    const float cmax = fmax(c.x, fmax(c.y, c.z));
 ...
 * @return float3 HSL
 */
inline float3 rgb2hsl(constant const float3& c)
{
    const float cmin = fmin(c.x, fmin(c.y, c.z));
    const float cmax = fmax(c.x, fmax(c.y, c.z));
                           ...
                           const float invGammaP)
{
    ...
    float deltaP = fsqrt_rn(float(dx * dx + dy * dy));
...
inline float CostYKfromLab(const float4 c1, const float4 c2, const float invGammaC)
{
    ...
    return expf(-(deltaC * invGammaC)); // Yoon & Kweon
}
