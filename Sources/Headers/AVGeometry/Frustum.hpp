...
#include <AVGeometry/HalfPlane.hpp>
...
struct Frustum
{
    ...
    bool isInfinite() const { return planes.size() == 4; }

    /// Return true if the Frustum is truncated
    bool isTruncated() const { return planes.size() == 6; }

    bool isPartiallyTruncated() const { return planes.size() == 5; }

    // Return the supporting frustum points (5 for the infinite, 8 for the truncated)
    const std::vector<Vec3>& frustum_points() const { return points; }
