...
int aliceVision_main(int argc, char* argv[])
{
    ...
    // Log Errors, if Metal is accessible
    #if ALICEVISION_IS_DEFINED(ALICEVISION_HAVE_METAL)

    for (const auto deviceID : depthMap::DeviceManager::getInstance().getDeviceIDs())
    {
        const auto& errMap = depthMap::DeviceManager::getInstance().getCommandManager(deviceID)->getErrors();
        if (errMap.empty())
        {
            ALICEVISION_LOG_INFO("No Metal command buffer errors on device " << deviceID);
            continue;
        }
        for (const auto& [cmdBuf, errStr] : errMap)
        {
            ALICEVISION_LOG_ERROR("[MTLDevice " << deviceID << "] An Error occured in command buffer " << cmdBuf << ":" << errStr);
        }
    }
